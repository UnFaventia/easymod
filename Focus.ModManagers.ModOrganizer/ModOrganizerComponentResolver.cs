...
public class ModOrganizerComponentResolver : IComponentResolver
{
    ...
    public ModOrganizerComponentResolver(
        IFileSystem fs,
        IModOrganizerConfiguration config,
        string rootPath
    )
    {
        this.config = config;
    ...
    public async Task<ModComponentInfo> ResolveComponentInfo(string componentName)
    {
        ...
        if (!fs.File.Exists(metaPath))
            return new ModComponentInfo(
                new ModLocatorKey(string.Empty, componentName),
                componentName,
                componentName,
                modPath,
                isEnabled
        ...
        if (
            ...
            && !string.IsNullOrEmpty(config.DownloadDirectory)
        )
        {
            var downloadMetaPath = fs.Path.Combine(
                config.DownloadDirectory,
                $"{installationFile}.meta"
            );
            try
            {
    ...
    private async Task<HashSet<string>?> ReadEnabledModNamesFromProfile()
    {
        if (string.IsNullOrEmpty(config.SelectedProfileName))
            return null;
        var modListPath = fs.Path.Combine(
