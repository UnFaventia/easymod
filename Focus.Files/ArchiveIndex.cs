...
public class ArchiveIndex : IBucketedFileIndex
{
    ...
    private readonly Dictionary<string, IReadOnlySet<string>> contentPaths =
        new(PathComparer.Default);
    private readonly Dictionary<string, HashSet<string>> inverseContentPaths =
        new(PathComparer.Default);
    ...
    public void AddArchives(IEnumerable<string> archivePaths)
    {
        ...
        var newEntries = newArchivePaths
            ...
            {
                ArchivePath = p,
                Files = archiveProvider
                    .GetArchiveFileNames(p)
    ...
    {
        return contentPaths.TryGetValue(bucketName, out var contents)
            && contents.Contains(pathInBucket);
    }
        ...
        if (inverseContentPaths.TryGetValue(pathInBucket, out var archivePaths))
        {
            ...
            return archivePaths
                ...
                .Select(p => new KeyValuePair<string, string>(
                    p,
                    PathComparer.NormalizePath(pathInBucket)
                ));
        }
    ...
    {
        return archiveOrder.Select(f => new KeyValuePair<string, IEnumerable<string>>(
            f,
            contentPaths[f]
        ));
    }
