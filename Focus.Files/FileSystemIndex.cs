...
public sealed class FileSystemIndex : INotifyingBucketedFileIndex, INotifyingFileIndex, IDisposable
{
    ...
    private readonly Dictionary<string, HashSet<string>> buckets =
    ...
    )
    {
        ...
        inverseBuckets = buckets
            ...
            {
                FilePath = g.Key,
                BucketNames = g.Select(x => x.BucketName)
    ...
    {
        return buckets.TryGetValue(bucketName, out var fileNames)
            ? fileNames
            : Enumerable.Empty<string>();
    }
    ...
    private void TrackFile(string absolutePath)
    {
        ...
        AddedToBucket?.Invoke(
            this,
            new BucketedFileEventArgs(bucketedPath.BucketName, bucketedPath.Path)
        );
    }
    ...
    private void UntrackFile(string absolutePath)
    {
        ...
        var bucketedPath = bucketize(relativePath);
        if (
            buckets.TryGetValue(bucketedPath.BucketName, out var bucket)
            && bucket.Remove(bucketedPath.Path)
        )
            RemovedFromBucket?.Invoke(
                this,
                new BucketedFileEventArgs(bucketedPath.BucketName, bucketedPath.Path)
            );
        if (inverseBuckets.TryGetValue(bucketedPath.Path, out var bucketNames))
