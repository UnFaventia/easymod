...
{
    private readonly ConcurrentDictionary<string, HashSet<string>> fileNameCache =
        new(PathComparer.Default);
    private readonly ConcurrentDictionary<
        string,
        ConcurrentDictionary<string, uint>
    > fileSizeCache = new(PathComparer.Default);
    ...
    public uint GetArchiveFileSize(string archivePath, string archiveFilePath)
    {
        ...
        return sizes.GetOrAdd(
            archiveFilePath,
            _ =>
                ContainsFile(archivePath, archiveFilePath)
                    ? innerProvider.GetArchiveFileSize(archivePath, archiveFilePath)
                    : 0
        );
    }
    ...
    {
        return fileNameCache.GetOrAdd(
            archivePath,
            _ => innerProvider.GetArchiveFileNames(archivePath).ToHashSet(PathComparer.Default)
        );
    }
}
