...
public class EnvironmentFactory : IEnvironmentFactory
{
    ...
    public GameEnvironmentState<ISkyrimMod, ISkyrimModGetter> CreateEnvironment()
    {
        ...
        var loadOrderKeys = setup
            .AvailablePlugins.Where(p =>
                setup.LoadOrderGraph.IsEnabled(p.FileName)
                && setup.LoadOrderGraph.CanLoad(p.FileName)
            )
        ...
        var creationClubFile = CreationClubListings.GetListingsPath(
            game.GameRelease.ToCategory(),
            setup.DataDirectory
        );
