...
public class MutagenArchiveProvider : IArchiveProvider
{
    ...
    private readonly ConcurrentDictionary<string, IArchiveReader> readers =
    ...
    public bool ContainsFile(string archivePath, string archiveFilePath)
    {
        return Safe(
            ...
            () =>
            {
                ...
                return reader.Files.Any(f =>
                    string.Equals(f.Path, archiveFilePath, StringComparison.OrdinalIgnoreCase)
                );
            }
        );
    }
    ...
    public IEnumerable<string> GetArchiveFileNames(string archivePath, string path = "")
    {
        return Safe(
                ...
                () =>
                {
                    ...
                    var reader = GetReader(archivePath);
                    return reader
                        .Files.Select(f => Safe(archivePath, () => f.Path))
                        .NotNull()
                        .Where(p =>
    ...
    public uint GetArchiveFileSize(string archivePath, string archiveFilePath)
    {
        ...
        return reader
            .Files.Select(f =>
                Safe(archivePath, () => new { f.Path, Size = new Lazy<uint>(() => f.Size) })
            )
            .NotNull()
    ...
    public bool IsArchiveFile(string path)
    {
        return string.Equals(
            ...
            archive.GetExtension(game.GameRelease),
    ...
    public ReadOnlySpan<byte> ReadBytes(string archivePath, string archiveFilePath)
    {
        ...
        if (string.IsNullOrEmpty(folderName))
            throw new ArgumentException(
                $"Archive path '{archivePath}' is missing directory info.",
                nameof(archivePath)
            );
        if (!reader.TryGetFolder(folderName, out var folder))
            throw new ArchiveException(
                $"Couldn't find folder {folderName} in archive {archivePath}"
            );
        var file = folder.Files.SingleOrDefault(f =>
            string.Equals(f.Path, archiveFilePath, StringComparison.OrdinalIgnoreCase)
        );
        if (file == null)
            throw new ArchiveException(
                $"Couldn't find file {archiveFilePath} in archive {archivePath}"
            );
        return file.GetSpan();
    }
    ...
    {
        return readers.GetOrAdd(
            archivePath,
            _ => archive.CreateReader(game.GameRelease, archivePath)
        );
    }
