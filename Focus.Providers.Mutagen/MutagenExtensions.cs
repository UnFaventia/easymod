...
{
    public static string GetRealDataDirectory<TModGetter>(
        this IReadOnlyGameEnvironment<TModGetter> env
    )
    ...
    {
        return LoquiRegistration.TryGetRegister(recordType, out var reg)
            ? reg.Name
            : recordType.Name;
    }
    ...
    )
    {
        ...
        Func<T, string> wrappedKeySelector = x => (keySelector(x) ?? FormKey.Null).ToString();
        return first
            .OrderBySafe(wrappedKeySelector)
            .SequenceEqualSafe(second.OrderBySafe(wrappedKeySelector));
    }
    ...
    public static IReadOnlyList<RecordKey> ToRecordKeys<T>(
        this IEnumerable<IFormLinkGetter<T>> formLinks
    )
        where T : class, IMajorRecordGetter
    {
        return formLinks
            .Select(x => x.FormKeyNullable)
            .NotNull()
            .Select(x => x.ToRecordKey())
            .ToList()
            .AsReadOnly();
    }
