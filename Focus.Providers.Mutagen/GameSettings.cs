...
public static class GameSettings
{
    public static GameSettings<TModGetter> From<TModGetter>(
        IReadOnlyGameEnvironment<TModGetter> env,
        GameSelection game
    )
    ...
    where TModGetter : class, IModGetter
{
    ...
    )
    {
        ...
        order = iniListings
            .Concat(
                env.LoadOrder.ListedOrder.SelectMany(x =>
                    new[]
                    {
    ...
    public IEnumerable<string> PluginLoadOrder =>
        env.LoadOrder.ListedOrder.Select(x => x.ModKey.FileName.String);
