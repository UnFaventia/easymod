...
{
    public static IEnumerable<IKeyValue<string, T>> AllFrom<T>(
        this IFormLinkGetter<T> link,
        IGroupCache cache
    )
        where T : class, ISkyrimMajorRecordGetter
    {
    ...
    public static ContextRelations<T> GetRelations<T>(
        this IGroupCache groups,
        IFormLink<T> link,
        string pluginName
    )
        where T : class, ISkyrimMajorRecordGetter
    {
        ...
        var masterNames =
            mod?.ModHeader.MasterReferences.Select(x => x.Master.FileName.String)
                .ToHashSet(StringComparer.CurrentCultureIgnoreCase) ?? new HashSet<string>();
        var previousContexts = link.AllFrom(groups)
            .SkipWhile(x =>
                !string.Equals(x.Key, pluginName, StringComparison.CurrentCultureIgnoreCase)
            )
            .Skip(1)
            .ToList();
        return new()
    ...
    where T : ISkyrimMajorRecordGetter
{
    ...
    public IEnumerable<IKeyValue<string, T>> Masters { get; init; } =
        Enumerable.Empty<IKeyValue<string, T>>();
