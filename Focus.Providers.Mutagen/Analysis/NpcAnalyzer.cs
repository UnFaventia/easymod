...
public class NpcAnalyzer : IRecordAnalyzer<NpcAnalysis>
{
    ...
    {
        private readonly ConcurrentDictionary<FormKey, HashSet<FormKey>> defaultRaceArmorAddons =
            new();
    ...
    private static readonly Func<INpcFaceMorphGetter, float>[] faceMorphValueSelectors = new Func<
        INpcFaceMorphGetter,
        float
    >[]
    {
    ...
    public NpcAnalyzer(
        IGroupCache groups,
        IArmorAddonHelper armorAddonHelper,
        IReferenceChecker<INpcGetter>? referenceChecker = null,
        IAssetPathExtractor<INpcGetter>? assetPathExtractor = null
    )
    {
    ...
    public NpcAnalysis Analyze(string pluginName, IRecordKey key)
    {
        ...
        var comparisonToMasters = relations
            .Masters.Select(x => Compare(npc, x.Value, x.Key))
            .ToList()
            .AsReadOnly();
        var comparisonToBase =
            FindComparison(comparisonToMasters, relations.Base?.Key)
            ?? Compare(npc, relations.Base?.Value, relations.Base?.Key);
        var comparisonToPrevious =
            FindComparison(comparisonToMasters, relations.Previous?.Key)
        ...
        return new()
        {
            ...
            ReferencedAssets = (
                assetPathExtractor?.GetReferencedAssets(npc) ?? Enumerable.Empty<AssetReference>()
            )
                .ToList()
                .AsReadOnly(),
    ...
    private static bool BehaviorsEqual(INpcGetter current, INpcGetter previous)
    {
        ...
        var sameExcludingConfigurationFlags = NpcsEqual(
            current,
            previous,
            mask =>
            {
        ...
        return sameExcludingConfigurationFlags
            ...
            current.Configuration.Equals(
                previous.Configuration,
                new NpcConfiguration.TranslationMask(true, true) { Flags = false }
            )
            &&
    ...
    private bool FacesEqual(INpcGetter x, INpcGetter y)
    {
        return x.Equals(
                y,
                new Npc.TranslationMask(false, false)
                {
                    FaceParts = true,
                    HairColor = true,
                    HeadTexture = true,
    ...
    private static NpcComparison? FindComparison(
        IEnumerable<NpcComparison> comparisons,
        string? pluginName
    )
    {
    ...
    private IEnumerable<IHeadPartGetter> GetMainHeadParts(INpcGetter npc)
    {
        ...
        var defaultHeadPartRefs = isFemale
            ? race?.HeadData?.Female?.HeadParts
            : race?.HeadData?.Male?.HeadParts;
        return (defaultHeadPartRefs ?? Enumerable.Empty<IHeadPartReferenceGetter>())
            ...
            .GroupBy(x => x.Type)
            .SelectMany(g =>
                IsSecondaryPart(g.Key) ? g.Cast<IHeadPartGetter>() : new[] { g.Last() }
            );
    }
    ...
    private NpcTemplateInfo? GetTemplateInfo(INpcGetter npc)
    {
        ...
        var inheritsTraits = npc.Configuration.TemplateFlags.HasFlag(
    ...
    private NpcWigInfo? GetWigInfo(INpcGetter npc)
    {
        ...
        var allAddons = wornArmor
            .Armature.Select(fk => fk.WinnerFrom(groups))
            .NotNull()
            .Where(x =>
                armorAddonHelper.SupportsRace(x, npc.Race)
                && !armorAddonHelper.IsDefaultSkin(x, npc.Race)
            )
            .ToList();
    ...
    private static bool NpcsEqual(
        INpcGetter x,
        INpcGetter y,
        Action<Npc.TranslationMask>? configure = null
    )
    {
    ...
    private static bool OutfitsEqual(INpcGetter x, INpcGetter y)
    {
        ...
        return x.DefaultOutfit.FormKey == y.DefaultOutfit.FormKey
    ...
    private static bool PlayerSkillsEqual(IPlayerSkillsGetter? x, IPlayerSkillsGetter? y)
    {
        ...
        return x.Equals(
                y,
                new PlayerSkills.TranslationMask(true)
                {
                    SkillOffsets = false,
                    SkillValues = false,
                    Unused = false,
    ...
    {
        return NearlyEquals(current.Height, previous.Height)
            && NearlyEquals(current.Weight, previous.Weight);
    }
    ...
    private static bool TintLayersEqual(
        IReadOnlyList<ITintLayerGetter> a,
        IReadOnlyList<ITintLayerGetter> b
    )
    {
        if (a.Count != b.Count)
            return false;
    ...
    private static bool TintLayersEqual(ITintLayerGetter x, ITintLayerGetter y)
    {
        ...
        return x.Equals(
            y,
            new TintLayer.TranslationMask(false, false)
            {
                Color = true,
                Index = true,
    ...
    private static bool VmadsEqual(IVirtualMachineAdapterGetter? x, IVirtualMachineAdapterGetter? y)
    {
        ...
        if (x is null || y is null)
            return false;
        return x.ObjectFormat == y.ObjectFormat
            && x.Version == y.Version
