...
public interface IAssetPathRecordConfigurationBuilder<T>
{
    ...
    IAssetPathRecordConfigurationBuilder<T> Add(
        AssetKind kind,
        Func<T, IEnumerable<string?>?> pathsSelector
    );
    IAssetPathRecordConfigurationBuilder<T> Add<TGendered>(
        AssetKind kind,
        Func<T, IGenderedItemGetter<TGendered?>?> genderedItemSelector,
...
public class AssetPathExtractor<T>
    : ReferenceFollower<T, AssetReferences, AssetReferences>,
        IAssetPathExtractor<T>
    where T : class, ISkyrimMajorRecordGetter
{
    ...
    private AssetPathExtractor(
        ...
        ISubAnalyzerBundle<AssetReferences> subAnalyzerBundle
    )
        : base(groupCache)
    {
    ...
    {
        return WalkAll(record).SelectMany(results => results).SelectMany(refs => refs).Distinct();
    }

    protected override AssetReferences Accumulate(
        AssetReferences? previous,
        AssetReferences current
    )
    {
        return current;
    }

    protected override ReferenceFollower<
        TNext,
        AssetReferences,
        AssetReferences
    > CreateChild<TNext>()
...
class AssetPathConfigurationBuilder : IAssetPathConfigurationBuilder
{
    ...
    public IAssetPathConfigurationBuilder For<T>(
        Action<IAssetPathRecordConfigurationBuilder<T>> config
    )
        where T : class, ISkyrimMajorRecordGetter
    {
    ...
    where T : class, ISkyrimMajorRecordGetter
{
    ...
    public IAssetPathRecordConfigurationBuilder<T> Add(
        AssetKind kind,
    ...
    {
        selectors.Add(x =>
            pathsSelector(x)?.NotNullOrEmpty().Select(path => new AssetReference(path, kind))
        );
        return this;
    }
