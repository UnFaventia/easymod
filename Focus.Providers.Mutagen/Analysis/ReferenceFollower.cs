    ...
    where T : class, ISkyrimMajorRecordGetter
{
    IReferenceFollower<T> Follow<TNext>(
        Func<T, IFormLinkGetter<TNext>?> linkSelector,
...
public abstract class ReferenceFollower<T, TAccumulate, TResult> : IReferenceFollower<T>
    where T : class, ISkyrimMajorRecordGetter
{
    protected ConcurrentDictionary<FormKey, TAccumulate> AccumulatorCache { get; private set; } =
        new();
    protected IGroupCache GroupCache { get; private init; }

    private readonly HashSet<string> excludedPluginNames =
        ...
        where TNext : class, ISkyrimMajorRecordGetter
    {
        return Follow(
            r =>
            {
                ...
                if (item is null)
                    return Enumerable.Empty<IFormLinkGetter<TNext>>();
                var subItems = new[] { item.Male, item.Female }
                    .Where(x => x is not null)
                    .Select(x => x!);
                return subItems.SelectMany(x =>
                    linksSelector(x) ?? Enumerable.Empty<IFormLinkGetter<TNext>>()
                );
            },
            configure
        );
        ...
        where TNext : class, ISkyrimMajorRecordGetter
    {
        ...
        routes.Add(
            (record, previous, visited) =>
                Walk(record, previous, linksSelector, subrecordFollower, visited)
        );
        return this;
    }
    ...
    public IReferenceFollower<T> FollowSelf(Func<T, IFormLinkGetter<T>?> linkSelector)
    {
        routes.Add(
            (record, previous, visited) =>
                Walk(record, previous, r => new[] { linkSelector(r) }, this, visited)
        );
        return this;
    }

    public IReferenceFollower<T> FollowSelf(
        Func<T, IEnumerable<IFormLinkGetter<T>?>?> linksSelector
    )
    {
        routes.Add(
            (record, previous, visited) => Walk(record, previous, linksSelector, this, visited)
        );
        return this;
    }
        ...
        where U : class, ISkyrimMajorRecordGetter
    {
        ...
        foreach (var link in linksSelector(record) ?? Enumerable.Empty<IFormLinkGetter<U>>())
        {
            ...
            var subrecordResults = subrecordFollower
                .routes.Select(ps => ps(nextRecord, result, visited))
