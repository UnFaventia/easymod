...
public static class ReferenceCheckerExtensions
{
    public static IReadOnlyList<ReferencePath> SafeCheck(
        this IReferenceChecker? checker,
    ...
    public static IReadOnlyList<ReferencePath> SafeCheck<T>(
        this IReferenceChecker<T>? checker,
        T? record
    )
        where T : class, ISkyrimMajorRecordGetter
    {
...
public class ReferenceChecker<T>
    : ReferenceFollower<T, ReferenceInfo, IEnumerable<ReferenceInfo>>,
        IReferenceChecker<T>
    where T : class, ISkyrimMajorRecordGetter
{
    ...
    {
        return WalkAll(record).SelectMany(paths => paths).Select(refs => new ReferencePath(refs));
    }
    ...
    public IEnumerable<ReferencePath> GetInvalidPaths(ISkyrimMajorRecordGetter record)
    {
        if (record is not T typedRecord)
            throw new ArgumentException(
                $"Expected a record of type {typeof(T).Name}, but got {record.GetType().Name}",
                nameof(record)
            );
        return GetInvalidPaths(typedRecord);
    }

    protected override IEnumerable<ReferenceInfo> Accumulate(
        IEnumerable<ReferenceInfo>? previous,
        ReferenceInfo current
    )
    {
