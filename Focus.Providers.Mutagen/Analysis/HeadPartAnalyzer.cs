...
public class HeadPartAnalyzer : IRecordAnalyzer<HeadPartAnalysis>
{
    ...
    public HeadPartAnalyzer(
        IGroupCache groups,
        IReferenceChecker<IHeadPartGetter>? referenceChecker = null
    )
    {
        this.groups = groups;
    ...
    private IEnumerable<VanillaRace> GetValidVanillaRaces(IHeadPartGetter headPart)
    {
        ...
        if (raceList is null)
            return Enumerable.Empty<VanillaRace>();
        return raceList
            .Items.Select(x => x.FormKey.ToLink<IRaceGetter>().WinnerFrom(groups))
            .NotNull()
