...
public class GroupCache : IGroupCache
{
    ...
    private readonly ConcurrentDictionary<
        Tuple<string, Type>,
        IReadOnlyCache<ISkyrimMajorRecordGetter, FormKey>?
    > commonGroups = new(new TupleEqualityComparer<string, Type>(StringComparer.OrdinalIgnoreCase));
    private readonly ConcurrentDictionary<
        Tuple<string, Type>,
        IGroupGetter<ISkyrimMajorRecordGetter>?
    > genericGroups =
        new(new TupleEqualityComparer<string, Type>(StringComparer.OrdinalIgnoreCase));
    private readonly ILogger log;
    private readonly ConcurrentDictionary<
        FormKey,
        IReadOnlyList<IKeyValue<string, object>>
    > records = new();
    private readonly ConcurrentDictionary<FormKey, IKeyValue<string, object>?> winningRecords =
        new();
    ...
    public IReadOnlyCache<ISkyrimMajorRecordGetter, FormKey>? Get(string pluginName, Type groupType)
    {
        ...
        return commonGroups.GetOrAdd(
            ...
            x =>
            {
                ...
                if (
                    genericGroups.TryGetValue(groupKey, out var genericGroup)
                    && genericGroup != null
                )
                    return genericGroup.RecordCache;
                var mod = GetMod(x.Item1);
                return mod?.TryGetTopLevelGroup(groupType)?.RecordCache
                    as IReadOnlyCache<ISkyrimMajorRecordGetter, FormKey>;
            }
        );
    }
    ...
    public IGroupGetter<T>? Get<T>(
        string pluginName,
        Func<ISkyrimModGetter, IGroupGetter<T>> groupSelector
    )
        where T : class, ISkyrimMajorRecordGetter
    {
        return (IGroupGetter<T>?)
            genericGroups.GetOrAdd(
                Tuple.Create(pluginName, typeof(T)),
                x =>
                {
                    var mod = GetMod(x.Item1);
    ...
    {
        return records
            .GetOrAdd(link.FormKey, _ => LoadAll(link).ToList().AsReadOnly())
        ...
        where T : class, ISkyrimMajorRecordGetter
    {
        ...
        return winningRecords.GetOrAdd(link.FormKey, _ => GetAll(link).FirstOrDefault())?.Value
            as T;
    }
