...
{
    Task ConfigureIndex(
        INotifyingBucketedFileIndex modIndex,
        string rootPath,
...
public class IndexedModRepository : IIndexedModRepository, IDisposable
{
    ...
    private readonly Dictionary<string, HashSet<string>> bucketNamesToArchivePaths =
        new(NameComparer);
    ...
    private Dictionary<string, ModComponentInfo> componentPathsToComponents =
    ...
    )
    {
        ...
        var bucketComponents = await componentResolver
            .ResolveAll(this.modIndex.GetBucketNames())
            .ConfigureAwait(false);
        bucketNamesToComponents = bucketComponents.ToDictionary(
            x => x.Key,
            x => x.Component,
            NameComparer
        );
        componentNamesToBucketNames = bucketNamesToComponents.ToDictionary(
            x => x.Value.Name,
            x => x.Key,
            NameComparer
        );
        componentNamesToComponents = bucketNamesToComponents.Values.ToDictionary(
            x => x.Name,
            NameComparer
        );
        componentPathsToComponents = bucketNamesToComponents.Values.ToDictionary(
            x => x.Path,
            PathComparer.Default
        );
        modIdsToComponents = bucketNamesToComponents
            .Values.GroupBy(x => x.ModKey.Id)
    ...
    {
        var looseFileComponents = modIndex
            .FindInBuckets(relativePath)
    ...
    public ModInfo? FindByKey(IModLocatorKey key)
    {
        if (
            ...
            && modIdsToComponents.TryGetValue(key.Id, out var modComponents)
        )
            return ConvertModInfo(key.Id, modComponents);
        if (
            !string.IsNullOrEmpty(key.Name)
            && modNamesToModIds.TryGetValue(key.Name, out var modId)
        ...
        if (
            !string.IsNullOrEmpty(key.Name)
            && componentNamesToComponents.TryGetValue(key.Name, out var component)
        )
    ...
    public ModInfo? GetById(string modId)
    {
        return
            ...
            ? ConvertModInfo(modId, modComponents)
            : null;
    }
    ...
    public ModInfo? GetByName(string modName)
    {
        if (string.IsNullOrEmpty(modName))
            return null;
        if (
            modNamesToModIds.TryGetValue(modName, out var modId)
            && modIdsToComponents.TryGetValue(modId, out var modComponents)
        )
            return ConvertModInfo(modId, modComponents);
    ...
    )
    {
        ...
        var archiveBuckets = includeArchives
            ? archiveIndex.FindInBuckets(relativePath)
        ...
        var archiveResults = archiveBuckets
            ...
            {
                x.ArchiveName,
                Component = x.BucketName is not null
                    ? bucketNamesToComponents.GetOrDefault(x.BucketName)
                    : null,
            })
            .Select(x =>
                x.Component is not null && (includeDisabled || x.Component.IsEnabled)
                    ? new ModSearchResult(x.Component, relativePath, x.ArchiveName)
                    : null
            )
            .NotNull();
    ...
    private ModInfo ConvertModInfo(string id, IEnumerable<ModComponentInfo> modComponents)
    {
        ...
        var modName = modIdsToModNames.TryGetValue(id, out var cachedName)
            ? cachedName
            : modComponents.First().ModKey.Name;
    ...
    {
        if (
            !string.IsNullOrEmpty(component.ModKey.Id)
    ...
    private async void ModIndex_AddedToBucket(object? sender, BucketedFileEventArgs e)
    {
        ...
        if (IsArchive(e.Path))
        {
            ...
            var archivePaths = bucketNamesToArchivePaths.GetOrAdd(
                e.BucketName,
                () => new(PathComparer.Default)
            );
    ...
    private void RegisterComponent(ModComponentInfo component, string bucketName)
    {
        if (string.IsNullOrEmpty(component.Name))
            throw new ModManagerException(
                "Cannot register a mod component without a component name."
            );
        ...
        if (
            !string.IsNullOrEmpty(component.ModKey.Id)
            && !modIdsToModNames.ContainsKey(component.ModKey.Id)
    ...
    private void SetupArchiveIndex()
    {
        ...
        var archivePathPairs = modIndex
            .GetBucketedFilePaths()
            .AsParallel()
            .Select(x => new
            {
                BucketName = x.Key,
                ArchivePaths = x
                    .Value.Where(path => IsArchive(path))
                    .Select(p => GetAbsolutePath(x.Key, p)),
            })
            .ToList();
        foreach (var archivePathPair in archivePathPairs)
            bucketNamesToArchivePaths.Add(
                archivePathPair.BucketName,
                archivePathPair.ArchivePaths.ToHashSet(PathComparer.Default)
            );
