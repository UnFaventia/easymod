...
public class LoadOrderAnalysis
{
    ...
    public IReadOnlyList<PluginAnalysis> Plugins { get; init; } =
        Empty.ReadOnlyList<PluginAnalysis>();
        ...
        where T : RecordAnalysis
    {
        return from plugin in Plugins
            let recordGroup = plugin.Groups.TryGetValue(recordType, out var foundGroup)
                ? foundGroup
                : null
