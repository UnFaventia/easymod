...
{
    private delegate RecordAnalysisGroup GroupAnalyzerAction(
        string pluginName,
        IEnumerable<IRecordKey> keys
    );
    ...
    public LoadOrderAnalysis Run(bool includeBasePlugins = false)
    {
        ...
        var pluginAnalyses = loadedPlugins
            ...
            .AsReadOnly();
        stopwatch.Stop();
        return new LoadOrderAnalysis { ElapsedTime = stopwatch.Elapsed, Plugins = pluginAnalyses };
    }
    ...
    private static GroupAnalyzerAction MakeAnalyzerAction<T>(IRecordAnalyzer<T> analyzer)
        where T : RecordAnalysis
    {
        return (pluginName, keys) =>
            new RecordAnalysisGroup<T>(
                analyzer.RecordType,
                keys.Select(key => analyzer.Analyze(pluginName, key))
            );
    }
