...
public class IndexedModRepositoryTests : IAsyncLifetime
{
    ...
    public IndexedModRepositoryTests()
    {
        ...
        componentResolverMock
            .Setup(x => x.ResolveComponentInfo(It.IsAny<string>()))
            .Returns(
                (string componentName) =>
                    Task.FromResult(
                        new ModComponentInfo(
                            componentName.IndexOf('_') == 2
                                ? new ModLocatorKey(
                                    componentName[0..2],
                                    modNames[componentName[0..2]]
                                )
                                : ModLocatorKey.Empty,
                            componentName.IndexOf('_') == 2
                                ? $"{componentName[3..]}_id"
    ...
    public void SearchForFiles_WhenArchivesRequested_YieldsMatchesInArchives()
    {
        ...
        Assert.Collection(
            modRepository.SearchForFiles(
                GetComponents("foo", "bar", "quux_disabled"),
                "common/c1",
                true,
                false
            ),
    ...
    public void SearchForFiles_WhenArchivesRequested_IgnoresArchivesInDisabledComponents()
    {
        ...
        Assert.Empty(
            modRepository.SearchForFiles(GetComponents("quux_disabled"), "quux3", true, false)
        );
    }
    ...
    public void SearchForFiles_WhenDisabledRequested_YieldsMatchesInDisabledComponents()
    {
        ...
        Assert.Collection(
            modRepository.SearchForFiles(
                GetComponents("foo", "quux_disabled"),
                "common/c1",
                false,
                true
            ),
