...
public class ComponentPerDirectoryModRepositoryTests
{
    ...
    public ComponentPerDirectoryModRepositoryTests()
    {
        ...
        indexedMock
            ...
            .Returns(new List<ModInfo> { DummyMod }.GetEnumerator());
    ...
    public async Task Configure_CreatesNewResolverAndConfiguresInnerIndex()
    {
        ...
        indexedMock.Verify(
            x =>
                x.ConfigureIndex(
                    It.Is<INotifyingBucketedFileIndex>(x =>
                        x.ToString() == @"Index:D:\new\root\path"
                    ),
                    @"D:\new\root\path",
    ...
    )
    {
        var components = new[]
        {
            new ModComponentInfo(ModLocatorKey.Empty, "cid", "cname", relativePath),
        };
        repository.ContainsFile(components, relativePath, includeArchives, includeDisabled);
        indexedMock.Verify(
            x => x.ContainsFile(components, relativePath, includeArchives, includeDisabled),
            Times.Once()
        );
    }
    ...
    )
    {
        ...
        indexedMock.Verify(
            x => x.ContainsFile(modInfo, relativePath, includeArchives, includeDisabled),
            Times.Once()
        );
    }
    ...
    [InlineData(@"path\to\file", false, true)]
    [InlineData(@"path\to\file", true, true)]
    public void SearchForFiles_Passthrough(
        string relativePath,
        bool includeArchives,
        bool includeDisabled
    )
    {
        ...
        var innerResults = new ModSearchResult[]
        {
            new ModSearchResult(dummyComponent, relativePath, null),
        };
        indexedMock
            .Setup(x => x.SearchForFiles(It.IsAny<string>(), It.IsAny<bool>(), It.IsAny<bool>()))
        ...
        indexedMock.Verify(
            x => x.SearchForFiles(relativePath, includeArchives, includeDisabled),
            Times.Once()
        );
    }
