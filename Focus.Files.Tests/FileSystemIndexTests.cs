...
public class FileSystemIndexTests : IDisposable
{
    ...
    public void WhenInitialized_IncludesExistingBuckets()
    {
        ...
        Assert.Equal(
            new[] { @"common\a.txt", "insub1.log", "insub1.txt" },
            index.GetFilePaths("sub1").OrderBy(f => f)
        );
        Assert.Equal(
            new[] { @"common\a.txt", "insub2.txt" },
            index.GetFilePaths("sub2").OrderBy(f => f)
        );
    ...
    public void GetContainingBucketNames_ProvidesAllBucketsWithFile(
        string pathInBucket,
        params string[] expectedBucketNames
    )
    {
        var results = index.FindInBuckets(pathInBucket);
        var assertions = expectedBucketNames.Select(bucketName =>
            (Action<KeyValuePair<string, string>>)(
                x =>
                {
    ...
    private static void AssertBucket(
        KeyValuePair<string, IEnumerable<string>> bucket,
        string name,
