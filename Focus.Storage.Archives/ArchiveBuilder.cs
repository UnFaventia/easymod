...
public class ArchiveBuilder
{
    ...
    )
    {
        var entries = Directory
            .GetFiles(directoryName, searchPattern ?? "*.*", SearchOption.AllDirectories)
    ...
    public BuildResult Build(string outputFileName)
    {
        ...
        Parallel.ForEach(
            ...
            group =>
            {
                ...
                using var manifest = new ArchiveManifest().AddAll(
                    group.Entries.Select(x => x.PathInArchive)
                );
                ...
                Parallel.ForEach(
                    ...
                    fileEntry =>
                    {
                        ...
                        Functions.BsaAddFileFromDisk(
                            archive,
                            fileEntry.PathInArchive,
                            fileEntry.LocalFilePath
                        );
    ...
    public ArchiveBuilder MaxCompressedSize(
        long maxCompressedSize,
        Func<FileEntry, int> estimateCompressedSize
    )
    {
    ...
    private IEnumerable<FileGroup> SplitEntries(string outputFileName)
    {
        ...
        foreach (var entry in fileEntries)
        {
            ...
            )
            {
                ...
                group = new FileGroup
                {
                    FileName = GetOverflowFileName(outputFileName, nextOverflowIndex++),
                };
            }
