...
class FakeGroupCache : IGroupCache
{
    ...
    public RecordKey[] AddRecords<T>(
        string pluginName,
        string masterName,
        params Action<T>[] configureRecords
    )
        where T : class, ISkyrimMajorRecordGetter
    {
    ...
    public FormKey FindKeyForEditorId(string editorId, out FakeReadOnlyCache group)
    {
        var found = groups
            .Values.SelectMany(g =>
                g.Where(x => x.Value.EditorID == editorId).Select(x => (g, x.Key))
            )
            .FirstOrDefault();
        group = found.g;
        return found.Key;
        ...
        where T : class, ISkyrimMajorRecordGetter, ILoquiObject
    {
        ...
        public ILoquiRegistration ContainedRecordRegistration =>
            LoquiRegistration.GetRegister(ContainedRecordType);
