...
public class HeadPartAnalyzerTests
    : CommonAnalyzerFacts<HeadPartAnalyzer, HeadPart, HeadPartAnalysis>
{
    public static TheoryData<string[], VanillaRace[]> RaceExtractionData =>
        new()
        {
            {
            ...
            { ["WoodElfRace", "OrcRace", "UnknownRace"], [VanillaRace.WoodElf, VanillaRace.Orc] },
        };
    ...
    public void ExtractsValidVanillaRacesFromEditorIds(
        IEnumerable<string> raceEditorIds,
        IEnumerable<VanillaRace> resolvedRaces
    )
    {
        var headPartKeys = Groups.AddRecords<HeadPart>(
            ...
            hp =>
            {
                var raceKeys = Groups.AddRecords(
                    "mod.esp",
                    raceEditorIds
                        .Select(editorId => (Action<Race>)(r => r.EditorID = editorId))
                        .ToArray()
                );
                var raceListKeys = Groups.AddRecords<FormList>(
                    "mod.esp",
                    f => f.Items.AddRange(raceKeys.ToFormKeys())
                );
    ...
    public void IncludesExtraParts()
    {
        ...
        var headPartKeys = Groups.AddRecords<HeadPart>(
            "mod.esp",
            hp => hp.ExtraParts.AddRange(extraPartKeys.ToFormKeys())
        );
        var analysis = Analyzer.Analyze("mod.esp", headPartKeys[0]);
    ...
    public void IncludesModelName()
    {
        var headPartKeys = Groups.AddRecords<HeadPart>(
            "mod.esp",
            hp => hp.Model = new() { File = "dir/model.nif" }
        );
        var analysis = Analyzer.Analyze("mod.esp", headPartKeys[0]);
    ...
    public void WhenHasExtraPartFlag_IsNotMainPart()
    {
        var headPartKeys = Groups.AddRecords<HeadPart>(
            "mod.esp",
            hp => hp.Flags |= HeadPart.Flag.IsExtraPart
        );
        var analysis = Analyzer.Analyze("mod.esp", headPartKeys[0]);
