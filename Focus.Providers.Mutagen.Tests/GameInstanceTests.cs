...
public class GameInstanceTests
{
    ...
    public void WhenInvalidGameId_ThrowsUnsupportedGame()
    {
        Assert.Throws<UnsupportedGameException>(
            () => GameInstance.FromGameId(gameLocations, "invalid")
        );
    }
    ...
    {
        Assert.Throws<MissingGameDataException>(
            () => GameInstance.FromGameId(gameLocations, "SkyrimSE")
        );
    }
    ...
    {
        gameLocationsMock
            .Setup(x => x.TryGetDataFolder(GameRelease.SkyrimSE, out It.Ref<DirectoryPath>.IsAny))
            .Callback(
                new TryGetDataFolderCallback(
                    (GameRelease gameRelease, out DirectoryPath path) =>
                    {
                        path = @"C:\path\to\game";
                    }
                )
