...
public class GameSetupTests
{
    ...
    {
        log = new LoggerConfiguration().MinimumLevel.Debug().WriteTo.Debug().CreateLogger();
        ...
        setupStaticsMock
            .Setup(x => x.GetBaseMasters(It.IsAny<GameRelease>()))
            .Returns(new List<ModKey>());
        setupStaticsMock
    ...
    public void WhenImplicitsIncludesPlugin_MarksAsImplicit()
    {
        ...
        listings.AddRange(
            CreateDummyListings("base1.esm", "base2.esp", "plugin1.esm", "plugin2.esp")
        );
        setupStaticsMock
            .Setup(x => x.GetBaseMasters(game.GameRelease))
            .Returns(
                new[]
                {
