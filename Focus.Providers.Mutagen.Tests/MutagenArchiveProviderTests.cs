...
public class MutagenArchiveProviderTests
{
    ...
    {
        archiveMock
            .Setup(x => x.CreateReader(GameRelease, "foo.bsa"))
            .Returns(
                new FakeArchiveReader().Put(@"a\b\c\1", "").Put(@"a\b\c\2", "").Put(@"a\b\c\3", "")
            );
        archiveMock
            .Setup(x => x.CreateReader(GameRelease, "bar.bsa"))
    ...
    public void GetArchiveFileNames_ReturnsFilesWithPathPrefix()
    {
        ...
        Assert.Equal(
            new[] { @"a\b\2", @"a\b\c\3", @"a\b\c\4" },
            provider.GetArchiveFileNames("foo.bsa", @"a\B\")
        );
        Assert.Equal(
            new[] { @"a\b\c\3", @"a\b\c\4" },
            provider.GetArchiveFileNames("foo.bsa", @"A\b/C")
        );
        Assert.Equal(
            new[] { @"d\5", @"d\6", @"d\e\7", @"d\e\f\8" },
            provider.GetArchiveFileNames("foo.bsa", @"d/")
        );
        Assert.Equal(
            new[] { @"d\e\7", @"d\e\f\8" },
            provider.GetArchiveFileNames("foo.bsa", @"d/e")
        );
    ...
    public void ReadBytes_ForDirectorylessPath_Throws()
    {
        Assert.Throws<ArgumentException>(
            () => provider.ReadBytes(@"C:\game\data\dummy.bsa", "rootfile.txt")
        );
    }
    ...
    public void ReadBytes_ForMissingFolder_Throws()
    {
        ...
        Assert.Throws<ArchiveException>(
            () => provider.ReadBytes(@"C:\game\data\archive.bsa", @"unknown\foo.bar")
        );
    }
    ...
    public void ReadBytes_ForMissingFile_Throws()
    {
        ...
        Assert.Throws<ArchiveException>(
            () => provider.ReadBytes(@"C:\game\data\archive.bsa", @"a\b\c\4")
        );
    }
    ...
    public void WhenArchiveHasInvalidData_FlagsAsBad()
    {
        archiveMock
            .Setup(x => x.CreateReader(GameRelease, "foo.bsa"))
            .Returns(new FakeArchiveReader().Put(@"a\b\c\1", "").MarkFolderCorrupted(@"a\b\c"));
