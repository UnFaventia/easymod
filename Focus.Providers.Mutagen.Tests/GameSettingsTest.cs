...
public class GameSettingsTest
{
    ...
    public GameSettingsTest()
    {
        ...
        archiveMock
            .Setup(x => x.GetIniListings(GameRelease))
            .Returns(new[] { new FileName("ini1.bsa"), new FileName("ini2.bsa") });
    ...
    public void ArchiveOrder_GetsFromArchiveService_UsingListingOrder()
    {
        ...
        archiveMock
            .Setup(x =>
                x.GetApplicableArchivePaths(
                    GameRelease,
                    gameDataPath,
                    It.IsAny<IEnumerable<FileName>>()
                )
            )
            .Returns(
                new[]
        ...
        archiveMock.Verify(x =>
            x.GetApplicableArchivePaths(GameRelease, @"C:\game\data", expectedOrdering)
        );
    }
    ...
    {
        Assert.Equal(
            new[] { "plugin1.esp", "plugin2.esp", "plugin3.esp" },
            settings.PluginLoadOrder
        );
    }
