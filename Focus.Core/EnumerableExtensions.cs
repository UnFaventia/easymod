...
public static class EnumerableExtensions
{
    ...
    )
    {
        ...
        return sequence
            ...
            .OrderBy(x =>
                loadOrderIndices.TryGetValue(x.key.BasePluginName, out var index) ? index : -1
            )
            .ThenBy(x =>
                int.TryParse(x.key.LocalFormIdHex, NumberStyles.HexNumber, null, out var id)
                    ? id
                    : -1
            )
            .Select(x => x.item);
    ...
    public static IEnumerable<T>? OrderBySafe<T, TKey>(
        this IEnumerable<T>? sequence,
    ...
    public static IEnumerable<T> ThenByLoadOrder<T>(
        this IOrderedEnumerable<T> sequence,
        Func<T, IRecordKey> keySelector,
        IEnumerable<string> loadOrder
    )
    {
        ...
        return sequence
            .ThenBy(x =>
                loadOrderIndices.TryGetValue(keySelector(x).BasePluginName, out var index)
                    ? index
                    : -1
            )
            .ThenBy(x =>
                int.TryParse(
                    keySelector(x).LocalFormIdHex,
