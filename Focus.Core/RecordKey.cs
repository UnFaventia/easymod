...
public class RecordKey : IRecordKey
{
    ...
    internal static readonly StringComparison DefaultComparison =
        StringComparison.CurrentCultureIgnoreCase;

    public string BasePluginName { get; private init; }
    public string LocalFormIdHex { get; private init; }

    public static RecordKey Parse(string value, bool natural = false)
    {
        ArgumentNullException.ThrowIfNull(value);
        var tokens = value.Split(':');
        if (tokens.Length != 2)
        {
            var expectedForm = natural ? "Plugin.esp:0123456" : "0123456:Plugin.esp";
            throw new ArgumentException(
                $"Invalid record key format: '{value}'. Must be of the form '{expectedForm}'.",
                nameof(value)
            );
        }
        ...
        return natural ? new RecordKey(tokens[0], tokens[1]) : new RecordKey(tokens[1], tokens[0]);
    }
    ...
    public RecordKey(IRecordKey key)
        : this(key.BasePluginName, key.LocalFormIdHex) { }
...
public class RecordKeyComparer : IEqualityComparer<IRecordKey>
{
    ...
    public bool Equals(IRecordKey? x, IRecordKey? y)
    {
        ...
        if (x is null || y is null)
            return false;
        return string.Equals(x.BasePluginName, y.BasePluginName, DefaultComparison)
    ...
    public int GetHashCode([DisallowNull] IRecordKey obj)
    {
        return HashCode.Combine(
            obj.BasePluginName.GetHashCode(DefaultComparison),
            obj.LocalFormIdHex.GetHashCode(DefaultComparison)
        );
    }
}
...
public static class RecordKeyExtensions
{
    public static bool PluginEquals(this IRecordKey recordKey, string pluginName)
    {
        return string.Equals(
            recordKey.BasePluginName,
            pluginName,
            RecordKeyComparer.DefaultComparison
        );
    }
}
