...
{
    private readonly Dictionary<string, LoadOrderNode> nodesByName =
    ...
    public IEnumerable<string> GetMissingMasters(string pluginName)
    {
        return nodesByName.TryGetValue(pluginName, out var node)
            ? GetMastersRecursive(node)
                .Where(x => !x.IsEnabled || !x.CanLoad)
    ...
    private IEnumerable<LoadOrderNode> GetMastersRecursive(
        LoadOrderNode node,
        HashSet<LoadOrderNode>? visitedNodes = null
    )
    {
