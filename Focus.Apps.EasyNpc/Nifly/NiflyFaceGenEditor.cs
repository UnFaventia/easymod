...
public class NiflyFaceGenEditor : IFaceGenEditor
{
    ...
    public IEnumerable<string> GetHeadPartNames(string referencePath, byte[] faceGenData)
    {
        ...
        var faceGenNode = faceGenFile
            .GetNodes()
            .FirstOrDefault(x => x.name.get() == "BSFaceGenNiNodeSkinned");
        if (faceGenNode is null)
        {
            log.Error(
                "FaceGen file '{faceGenPath}' cannot be processed because it is missing a BSFaceGenNiNodeSkinned "
                    + "node. Either it is not a valid FaceGen file or it is for the wrong game.",
                referencePath
            );
            return Enumerable.Empty<string>();
        }
    ...
    )
    {
        ...
        {
            if (
                string.IsNullOrEmpty(addedPart.EditorId) || string.IsNullOrEmpty(addedPart.FileName)
            )
            {
                log.Error(
            ...
            if (
                hairColorNullable is Color hairColor
                && headPartClone.HasShaderProperty()
            ...
            {
                shader.hairTintColor = new Vector3(
                    hairColor.R / 255f,
                    hairColor.G / 255f,
                    hairColor.B / 255f
                );
    ...
    private static IEnumerable<NiShape> GetChildShapes(NifFile file, NiNode parent)
    {
        ...
        return refs.Select(x => header.GetBlockById(x.index))
            .Where(x =>
                x is BSDynamicTriShape
                || /* Oldrim and bad SE conversions */
                x is NiTriShape
            )
            .Cast<NiShape>();
    }
