...
public class LineupBuilder : ILineupBuilder, IDisposable
{
    ...
    public LineupBuilder(
        ...
        IMugshotRepository mugshotRepository,
    ...
    )
    {
        ...
        appSettings
            .MugshotRedirectsObservable.TakeUntil(disposed)
            .Subscribe(x => modSynonyms = x.ToLookup(r => r.ModName, r => r.Mugshots));
    }

    public async IAsyncEnumerable<Mugshot> Build(
        INpcBasicInfo npc,
        IEnumerable<string> affectingPlugins
    )
    {
        ...
        {
            var mugshotFile = GetMugshotFile(
                mugshotFiles,
                pluginGroup.ModKey,
                npc.IsFemale,
                modSynonyms
        ...
        var facegenGroups = modRepository
    ...
    private Mugshot CreateMugshotModel(
        MugshotFile file,
        IModLocatorKey? modKey,
        IEnumerable<ModComponentInfo>? components = null,
    ...
    )
    {
        ...
        return new Mugshot
        {
            InstalledComponents = (components ?? Enumerable.Empty<ModComponentInfo>())
                .ToList()
                .AsReadOnly(),
            InstalledMod = mod,
    ...
    private MugshotFile GetMugshotFile(
        ILookup<string, MugshotFile> providedFiles,
        IModLocatorKey? modKey,
        bool female,
    ...
    )
    {
        ...
        if (exactMatch is not null)
            return exactMatch;
        var synonymMatch = mod
            .Components.Select(x => x.Name)
            .Prepend(mod.Name)
