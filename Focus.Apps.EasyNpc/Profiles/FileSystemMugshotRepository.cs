...
public class FileSystemMugshotRepository : IMugshotRepository, IDisposable
{
    ...
    private Task<IBucketedFileIndex> indexTask = Task.FromResult<IBucketedFileIndex>(
        new EmptyFileIndex()
    );

    public FileSystemMugshotRepository(
        IFileSystem fs,
        IObservableAppSettings settings,
    ...
    )
    {
        ...
        settings.MugshotsDirectoryObservable.TakeUntil(disposed).Subscribe(RebuildIndex);
    }
    ...
    {
        var searchPaths = extensions.Select(ext =>
            fs.Path.Combine(npcKey.BasePluginName, $"00{npcKey.LocalFormIdHex}{ext}")
        );
        var index = await indexTask;
        var bucketedResults = searchPaths.SelectMany(path => index.FindInBuckets(path));
        return bucketedResults.Select(x => new MugshotFile(
            string.Empty,
            x.Key,
            fs.Path.Combine(currentDirectoryPath, x.Key, x.Value)
        ));
    }
