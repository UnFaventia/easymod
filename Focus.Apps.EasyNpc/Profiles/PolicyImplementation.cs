...
public class ProfilePolicy : IProfilePolicy, ILoadOrderAnalysisReceiver
{
    ...
    public NpcSetupAttributes GetSetupRecommendation(INpcBasicInfo npc)
    {
        ...
        var faceGuess =
            chain.LastOrDefault(x => x.Analysis.ComparisonToBase?.ModifiesFace == true) ?? chain[0];
        ...
        var resolvedDefaultA = FindEarliestSource(
            chain,
            defaultGuessA,
            true,
            x => !x.ModifiesBehavior
        );
    ...
    {
        if (
            !npcChain.Master.CanUseFaceGen
            || npcChain.Master.IsChild
            || npcChain.Master.IsAudioTemplate
        )
            return false;
    ...
    {
        baseNames = analysis
            .Plugins.Where(x => x.IsBaseGame)
            .Select(x => x.FileName)
            .ToHashSet(StringComparer.CurrentCultureIgnoreCase);
        overhaulNames = InferOverhaulNames(analysis)
            .ToHashSet(StringComparer.CurrentCultureIgnoreCase);
        npcChains = analysis
    ...
    private static NpcComparison? FindEarliestComparison(
        NpcAnalysis npc,
        Predicate<NpcComparison> predicate,
        bool alwaysCheckPreviousOverride
    )
    {
        ...
        {
            if (
                comparison != npc.ComparisonToBase
        ...
        if (
            alwaysCheckPreviousOverride
            && npc.ComparisonToPreviousOverride is not null
    ...
    private Sourced<NpcAnalysis> FindEarliestSource(
        ...
        Sourced<NpcAnalysis> referenceRecord,
        bool alwaysCheckPreviousOverride,
        Predicate<NpcComparison> sourcePredicate,
    ...
    )
    {
        ...
        while (true)
        {
            ...
            var match = FindEarliestComparison(
    ...
    private static IEnumerable<string> InferOverhaulNames(LoadOrderAnalysis analysis)
    {
        ...
        var pluginStats = analysis
            .Plugins.Where(p => !p.IsBaseGame && p.Groups.ContainsKey(RecordType.Npc))
            .Select(p => new
            {
                p.FileName,
                Npcs = p.Groups[RecordType.Npc].Records.OfType<NpcAnalysis>().ToList(),
            })
            .Select(p => new
            {
                p.FileName,
        ...
        return pluginStats
            ...
            .Where(p =>
                p.TotalNpcCount < 3
                || (double)p.NewNpcCount / p.TotalNpcCount < newNpcPercentThreshold
            )
            .Where(p =>
                p.FaceModifierCount == p.TotalNpcCount
                || (
                    p.BehaviorlessFaceModifierCount
                    >= p.FaceModifierCount * behaviorlessModifierPercentThreshold
                )
            )
            .Select(p => p.FileName);
    }
