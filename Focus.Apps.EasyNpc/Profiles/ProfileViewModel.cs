...
public class ProfileViewModel
{
    ...
    )
    {
        ...
        pluginOrder = gameSettings
            .PluginLoadOrder.Select((pluginName, index) => (pluginName, index))
    ...
    private void ApplyFilters()
    {
        ...
        if (!string.IsNullOrEmpty(Filters.SelectedDefaultPlugin))
            filteredNpcs = filteredNpcs.Where(x =>
                x.IsDefaultPlugin(Filters.SelectedDefaultPlugin)
            );
        ...
        filteredNpcs = filteredNpcs.Where(x =>
            x.GetOverrideCount(!Filters.NonDlc) >= minOverrideCount || x.HasAvailableModdedFaceGens
        );
        filteredNpcs = filteredNpcs
            ...
            .ThenBy(x =>
                uint.TryParse(x.LocalFormIdHex, NumberStyles.HexNumber, null, out var formId)
                    ? formId
                    : uint.MaxValue
            );
    ...
    private void ApplySearchParameter(
        ref IEnumerable<INpc> npcs,
        Func<INpcSearchParameters, string> propertySelector
    )
    {
        ...
        if (string.IsNullOrEmpty(filterText))
            return;
        npcs = npcs.Where(x =>
            propertySelector(x)?.Contains(filterText, StringComparison.CurrentCultureIgnoreCase)
            ?? false
        );
    }
