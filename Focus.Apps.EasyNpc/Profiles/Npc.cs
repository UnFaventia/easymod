...
public interface INpc : INpcBasicInfo
{
    ...
    void ApplyPolicy(
        bool resetDefaultPlugin = false,
        bool resetFacePlugin = false,
    ...
    NpcChangeResult SetFaceOption(
        string pluginName,
        bool keepFaceGenMod = false,
...
public class Npc : INpc
{
    ...
    public bool HasAvailableFaceCustomizations =>
        Options.Any(x => x.Analysis.ComparisonToBase?.ModifiesFace == true)
        || HasAvailableModdedFaceGens;
    ...
    public bool HasMissingPlugins =>
        !string.IsNullOrEmpty(MissingDefaultPluginName)
        || !string.IsNullOrEmpty(MissingFacePluginName);
    ...
    public Npc(
        RecordAnalysisChain<NpcAnalysis> records,
        IReadOnlySet<string> baseGamePluginNames,
        IModRepository modRepository,
        IProfileEventLog profileEventLog,
    ...
    )
    {
        ...
        hasAvailableModdedFaceGens = new(
            () =>
            {
                var masterComponentNames = modRepository
                    .SearchForFiles(records.Master.BasePluginName, false)
                ...
                return modRepository
                    .SearchForFiles(faceGenPath, true)
    ...
    )
    {
        ...
        if (resetDefaultPlugin)
            if (
                SetDefaultOption(setupAttributes.DefaultPluginName) == NpcChangeResult.Redundant
                && alwaysLog
            )
                LogProfileEvent(
                    NpcProfileField.DefaultPlugin,
                    previousDefaultPlugin,
                    DefaultOption.PluginName
                );
        if (resetFacePlugin)
            if (
                SetFaceOption(setupAttributes.FacePluginName) == NpcChangeResult.Redundant
                && alwaysLog
            )
                LogProfileEvent(
                    NpcProfileField.FacePlugin,
                    previousFacePlugin,
                    FaceOption.PluginName
                );
    }
    ...
    public IEnumerable<string> GetFaceModNames()
    {
        ...
        return modRepository
            .SearchForFiles(FaceOption.PluginName, false)
            .Select(x => x.ModKey.Name);
    }
    ...
    {
        return DefaultOption.PluginName.Equals(
            pluginName,
    ...
    {
        var option = Options.Where(x => x.IsBaseGame).LastOrDefault();
    ...
    public NpcChangeResult SetDefaultOption(string pluginName, bool asFallback = false)
    {
        ...
        if (
            option is not null
            && (option != DefaultOption || !string.IsNullOrEmpty(MissingDefaultPluginName))
        )
        {
            var oldPluginName = !string.IsNullOrEmpty(MissingDefaultPluginName)
                ? MissingDefaultPluginName
                : DefaultOption.PluginName;
            if (!asFallback)
    ...
    public NpcChangeResult SetFaceMod(string modName)
    {
        var mod = ModLocatorKey.TryParse(modName, out var key)
            ? modRepository.FindByKey(key)
            : modRepository.GetByName(modName);
        if (mod is null)
            return NpcChangeResult.Invalid;
        var bestOption = Options.LastOrDefault(x =>
            modRepository.ContainsFile(mod, x.PluginName, false)
        );
        if (
            bestOption is null
            && !modRepository.ContainsFile(mod, FileStructure.GetFaceMeshFileName(this), true)
        )
            return NpcChangeResult.Invalid;
        if (
            (FaceGenOverride is not null && FaceGenOverride.IncludesName(modName))
            || modRepository.ContainsFile(mod, FaceOption.PluginName, false)
        )
            return NpcChangeResult.Redundant;
        return bestOption is not null
            ? SetFaceOption(bestOption.PluginName)
            : SetFaceGenOverride(mod);
    }

    public NpcChangeResult SetFaceOption(
        string pluginName,
        bool keepFaceGenMod = false,
        bool asFallback = false
    )
    ...
    public void WriteToEventLog()
    {
        ...
        if (FaceGenOverride is not null)
            LogProfileEvent(
                NpcProfileField.FaceMod,
                null,
                new ModLocatorKey(FaceGenOverride).ToString()
            );
    }
    ...
    private bool CheckUnmodifiedFaceTemplate()
    {
        ...
        {
            if (
                option.Analysis.TemplateInfo is null
                || !option.Analysis.TemplateInfo.InheritsTraits
            )
                return false;
            var currentTemplateKey = option.Analysis.TemplateInfo.Key;
            if (
                previousTemplateKey is not null
                && !keyComparer.Equals(previousTemplateKey, currentTemplateKey)
            )
                return false;
