...
#pragma warning restore 67
    ...
    public NpcViewModel(INpc npc, IAsyncEnumerable<Mugshot> mugshots)
    {
        ...
        Mugshots = mugshots
            .Select(x => new MugshotViewModel(
                x,
                npc.Options,
                IsSelectedMugshot(x.ModName, x.InstalledPlugins)
            ))
            .ToObservableCollection();
        isInitialized = true;
    }

    public bool TrySetDefaultPlugin(
        string pluginName,
        [MaybeNullWhen(false)] out NpcOptionViewModel option
    )
    {
    ...
    public bool TrySetFaceMod(
        MugshotViewModel mugshot,
        [MaybeNullWhen(false)] out NpcOptionViewModel option
    )
    {
