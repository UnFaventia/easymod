...
public interface IProfileFactory
{
    ...
    Profile RestoreSaved(
        ...
        out IReadOnlyList<NpcRestorationFailure> failures,
...
public class ProfileFactory : IProfileFactory
{
        ...
        try
        {
            profile = Create(
                ...
                npc =>
                {
                    ...
                    var isDefaultPluginInvalid =
                        newestEvents.TryGetValue(
                            (npcKey, NpcProfileField.DefaultPlugin),
                            out var defaultPluginEvent
                        )
                        && npc.SetDefaultOption(defaultPluginEvent.NewValue ?? string.Empty)
                            == NpcChangeResult.Invalid;
                    var isFacePluginInvalid =
                        newestEvents.TryGetValue(
                            (npcKey, NpcProfileField.FacePlugin),
                            out var facePluginEvent
                        )
                        && npc.SetFaceOption(facePluginEvent.NewValue ?? string.Empty)
                            == NpcChangeResult.Invalid;
                    ...
                    if (
                        newestEvents.TryGetValue(
                            (npcKey, NpcProfileField.FaceMod),
                            out var faceModEvent
                    ...
                    )
                    {
                        var faceMod = ModLocatorKey.TryParse(faceModEvent.NewValue, out var key)
                            ...
                            : modRepository.GetByName(faceModEvent.NewValue);
                        var isFaceGenOverride =
                            faceMod is not null
                            && !npc.Options.Any(x =>
                                modRepository.ContainsFile(faceMod, x.PluginName, false)
                            );
                        isFaceModInvalid =
                            isFaceGenOverride
                            && npc.SetFaceMod(faceModEvent.NewValue) == NpcChangeResult.Invalid;
                    }
                    var allEvents = new[]
                    {
                        defaultPluginEvent,
            ...
            foreach (var failure in failuresList)
            {
                ...
                if (failure.IsDefaultPluginInvalid)
                    failure.Npc.SetDefaultOption(
                        setupAttributes.DefaultPluginName,
                        asFallback: true
                    );
    ...
    private Profile Create(
        LoadOrderAnalysis analysis,
        IProfilePolicy policy,
        Action<Npc> defaultAction
    )
    {
        var baseGamePluginNames = analysis
