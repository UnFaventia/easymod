...
public class LoaderModel
{
    ...
    public LoaderModel(
        ...
        IGameSetup setup,
        Lazy<ILoadOrderAnalyzer> analyzer,
        IProfileFactory profileFactory,
        IConfigurableModRepository<ComponentPerDirectoryConfiguration> modRepository,
        IEnumerable<ILoadOrderAnalysisReceiver> analysisReceivers,
        ILogger logger
    )
    {
    ...
    public LoaderTasks Complete()
    {
        ...
        setup.Confirm();
        var modRepositoryTask = modRepositoryConfigureTask.ContinueWith(_ =>
            modRepository as IModRepository
        );
        var loadOrderAnalysisTask = AnalyzeLoadOrder();
        var profileTask = Task.WhenAll(loadOrderAnalysisTask, modRepositoryTask)
            .ContinueWith(t =>
                profileFactory.RestoreSaved(loadOrderAnalysisTask.Result, out _, out _)
            );
    ...
    public void Prepare()
    {
        modRepositoryConfigureTask = !string.IsNullOrEmpty(modSettings.RootDirectory)
            ...
            {
                log.Information(
                    "Beginning mod indexing in {modRootDirectory}",
                    modSettings.RootDirectory
                );
