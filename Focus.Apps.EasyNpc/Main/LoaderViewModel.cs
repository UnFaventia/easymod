...
public class LoaderViewModel
{
    ...
    public bool IsUnloadablePluginWarningVisible =>
        HasEnabledUnloadablePlugins && IsPluginListVisible;
    ...
    public LoaderViewModel(LogViewModel logViewModel, IGameSetup setup, LoaderModel loader)
    {
        ...
        Plugins = setup
            .AvailablePlugins.Select(
                (x, i) => new PluginSetting(x.FileName, i + 1, x.IsReadable, x.IsEnabled)
            )
            .ToList()
            .AsReadOnly();
        UpdatePluginStates();
    ...
    public async void ConfirmPlugins()
    {
        ...
        await Task.WhenAll(
                new Task[] { Tasks.ModRepository, Tasks.LoadOrderAnalysis, Tasks.Profile }
            )
