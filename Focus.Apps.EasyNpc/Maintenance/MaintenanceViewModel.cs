...
public class MaintenanceViewModel
{
    ...
    public void DeleteOldLogFiles()
    {
        ...
        try
        {
            var logFileNames = Directory
                .GetFiles(ProgramData.DirectoryPath, "Log_*.txt")
    ...
    private Predicate<INpc> GetResetPredicate(NpcProfileField field)
    {
        ...
        return field switch
        {
            NpcProfileField.DefaultPlugin => npc =>
                !string.IsNullOrEmpty(npc.MissingDefaultPluginName),
    ...
    private void RefreshLogStats()
    {
        ...
        LogFileSizeMb = (decimal)
            Math.Round(logFileNames.Select(f => new FileInfo(f).Length / 1024f / 1024f).Sum(), 1);
    }

    private void RefreshProfileStats()
    {
        var profileEvents = ProfileEventLog
            .ReadEventsFromFile(ProgramData.ProfileLogFileName)
            .ToList();
