...
public class ModSettings : IObservableModSettings
{
    ...
    )
    {
        ...
        RootDirectoryObservable = Observable
            .CombineLatest(
                appSettings.DefaultModRootDirectoryObservable,
                appSettings.UseModManagerForModDirectoryObservable,
                (defaultModRootDirectory, useModManagerDefault) =>
                    (defaultModRootDirectory, useModManagerDefault)
            )
    ...
    {
        return GetModRootDirectory(
            appSettings.DefaultModRootDirectory,
            appSettings.UseModManagerForModDirectory
        );
    }
    ...
    private string GetModRootDirectory(string defaultModRootDirectory, bool useModManagerDefault)
    {
        ...
        return !string.IsNullOrEmpty(modManagerConfig.ModsDirectory)
            ? modManagerConfig.ModsDirectory
            : defaultModRootDirectory;
    }
}
