...
public class SettingsViewModel
{
    ...
    public IEnumerable<string> AvailableMugshotModNames { get; private set; } =
    ...
    public SettingsViewModel(
        IMutableAppSettings settings,
        IModManagerConfiguration modManagerConfig,
        IModRepository modRepository,
        IAfterGameSetup<IGameSettings> gameSettingsFuture,
    ...
    )
    {
        ...
        var buildWarningSuppressions = settings.BuildWarningWhitelist.Select(
            x => new BuildWarningSuppressionViewModel(x.PluginName, x.IgnoredWarnings)
        );
        BuildWarningWhitelist = new(buildWarningSuppressions);
        WatchCollection(BuildWarningWhitelist, SaveBuildWarningSuppressions);

        var mugshotRedirects = settings.MugshotRedirects.Select(x => new MugshotRedirectViewModel(
            x.ModName,
            x.Mugshots
        ));
    ...
    public void SelectModRootDirectory(Window owner)
    {
        const string description =
            "Select root directory where your individual mod directories are located";
    ...
    protected void OnMugshotsDirectoryChanged()
    {
        AvailableMugshotModNames = Directory.Exists(MugshotsDirectory)
            ? Directory
                .GetDirectories(MugshotsDirectory)
...
#pragma warning restore 67
    ...
    public IEnumerable<BuildWarningId> SelectedWarnings { get; private set; } =
        Enumerable.Empty<BuildWarningId>();
    public IReadOnlyList<BuildWarningSelection> WarningSelections { get; init; }

    public BuildWarningSuppressionViewModel(
        string? pluginName,
        IEnumerable<BuildWarningId>? warnings = null
    )
    {
