...
public partial class App : Application
{
    ...
    private void Application_Startup(object sender, StartupEventArgs e)
    {
        ...
        Parser
            .Default.ParseArguments<CommandLineOptions>(e.Args)
            .WithParsed(Start)
    ...
    private void Start(CommandLineOptions options)
    {
        ...
        var isFirstLaunch =
            options.ForceIntro
            || (
                startupInfo.ModDirectoryOwner == ModManager.None
                && !File.Exists(ProgramData.ProfileLogFileName)
            );
        var isVortex =
            startupInfo.Launcher == ModManager.Vortex
            || startupInfo.ModDirectoryOwner == ModManager.Vortex;
        if (
            isVortex
            && string.IsNullOrEmpty(options.VortexManifest)
        ...
        if (isFirstLaunch && string.IsNullOrEmpty(Settings.Default.DefaultModRootDirectory))
            Settings.Default.DefaultModRootDirectory = container
                .Resolve<IModManagerConfiguration>()
                .ModsDirectory;
        try
        {
            try
    ...
    private bool Warn(StartupWarning warning, bool isFatal = false)
    {
        ...
        var warningWindow = new StartupWarningWindow { DataContext = model };
