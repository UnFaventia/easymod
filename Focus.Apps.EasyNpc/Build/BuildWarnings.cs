...
public class BuildWarning
{
    ...
    public BuildWarning(BuildWarningId id, string message)
        : this(message)
    {
        Id = id;
    }

    public BuildWarning(RecordKey key, BuildWarningId id, string message)
        : this(id, message)
    {
        RecordKey = key;
    }
    ...
    private static BuildWarningSeverity GetSeverity(BuildWarningId? id) =>
        id switch
        {
...
public static class WarningMessages
{
    ...
    {
        return $"{NpcLabel(editorId, name)} is configured to use mod '{modName}' as a FaceGen override, which "
    ...
    {
        return $"Plugin '{pluginName}' makes edits to {NpcLabel(editorId, name)} that require a FaceGen file, but "
    ...
    {
        return $"FaceGen file for {NpcLabel(editorId, name)} is being provided by multiple mods/components: "
    ...
    public static string SelectedPluginRemoved(
        string editorId,
        string name,
        string fieldName,
        string pluginName
    ...
    {
        return $"{NpcLabel(editorId, name)} references missing or disabled {pluginName} for its {fieldName} plugin "
            + $"selection.";
    }

    public static string WigNotMatched(
        string editorId,
        string name,
        string pluginName,
        string modelName
    ...
    {
        return $"No matching hair for wig model '{modelName}' used by {NpcLabel(editorId, name)} in {pluginName}.";
    }
