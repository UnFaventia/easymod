...
{
    public delegate AlertsViewModel Factory(
        Profile profile,
        IObservable<BuildSettings> buildSettings
    );

    public IReadOnlyList<BuildWarning> GlobalWarnings { get; private set; } =
        new List<BuildWarning>().AsReadOnly();
    public IEnumerable<SummaryItem> HeaderSummaryItems { get; private set; } =
        Enumerable.Empty<SummaryItem>();
    public IEnumerable<SummaryItem> MainSummaryItems { get; private set; } =
        Enumerable.Empty<SummaryItem>();
    public IReadOnlyList<BuildWarning> NpcWarnings { get; private set; } =
        new List<BuildWarning>().AsReadOnly();
    ...
    public AlertsViewModel(
        ...
        IObservableModSettings modSettings,
        IEnumerable<IGlobalBuildCheck> globalChecks,
    ...
    public void BeginWatching()
    {
        ...
        throttledBuildSettings
            ...
            .SubscribeSafe(
                ...
                settings =>
                {
                    ...
                    NpcWarnings = npcWarnings
                        .Values.SelectMany(warnings => warnings)
                        .ToList()
                        .AsReadOnly();
                    UpdateResults();
                }
            );
        var throttledModSettings = modSettings.RootDirectoryObservable.Throttle(
            TimeSpan.FromSeconds(1)
        );
        Observable
            .CombineLatest(throttledBuildSettings, throttledModSettings, (settings, _) => settings)
            .ObserveOn(NewThreadScheduler.Default)
            .TakeUntil(disposed)
            .SubscribeSafe(
                log,
        ...
        var modifiedNpcs = Observable.Merge(
            profile.Npcs.SelectMany(npc =>
                new[]
                {
        ...
        appSettings
            .BuildWarningWhitelistObservable.TakeUntil(disposed)
    ...
    {
        return appSettings
            .BuildWarningWhitelist.SelectMany(x =>
                x.IgnoredWarnings.Select(id => new { Plugin = x.PluginName, Id = id })
            )
    ...
    private static SummaryItemCategory PickCategory(
        int issueCount,
        SummaryItemCategory nonZeroCategory
    )
    {
    ...
    {
        GlobalWarnings = globalChecks
            .SelectMany(x => x.Run(profile, settings))
            .ToList()
            .AsReadOnly();
    }
    ...
    private void UpdateSummaryItems()
    {
        ...
        var suppressedCount =
            GlobalWarnings.Count + NpcWarnings.Count - criticalCount - conflictCount - otherCount;
        HeaderSummaryItems = new SummaryItem[]
        {
            new(
                PickCategory(criticalCount, SummaryItemCategory.StatusError),
                "Critical issues",
                criticalCount
            ),
            new(
                PickCategory(conflictCount, SummaryItemCategory.StatusWarning),
                "NPC conflicts",
                conflictCount
            ),
        };
        ...
        {
            new(
                PickCategory(otherCount, SummaryItemCategory.StatusInfo),
                "Other issues",
                otherCount
            ),
