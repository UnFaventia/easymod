...
public class BuildPreviewViewModel : IDisposable
{
    ...
    public BuildPreviewViewModel(
        ...
        NpcSummaryViewModel.Factory npcsFactory,
        PluginsViewModel.Factory pluginsFactory,
        AlertsViewModel.Factory alertsFactory,
        OutputViewModel.Factory outputFactory,
        AssetsViewModel.Factory assetsFactory,
        ILogger log,
        Profile profile,
    ...
    )
    {
        ...
        Observable
            .CombineLatest(
                Assets.OverallErrorLevel,
                Output.OverallErrorLevel,
                Alerts.OverallErrorLevel
            )
            .Select(errorLevels => errorLevels.Max())
            .TakeUntil(disposed)
            .SubscribeSafe(log, lvl => OverallErrorLevel = lvl);
        Output
            .BuildSettings.TakeUntil(disposed)
    ...
    {
        messageBus.Send(
            new JumpToProfile(new JumpToProfile.FilterOverrides { DefaultPlugin = pluginName })
        );
    }
