...
public partial class BuildPreviewView : UserControl
{
    ...
    private void ContentScrollViewer_ScrollChanged(object sender, ScrollChangedEventArgs e)
    {
        ...
        try
        {
            ...
            var bestExpander =
                expanders.FirstOrDefault(IsFullyVisible) ?? expanders.FirstOrDefault(IsTopVisible);
    ...
    private bool IsFullyVisible(FrameworkElement element)
    {
        ...
        var elementBottom = element
            .TranslatePoint(new(0, element.ActualHeight), ContentScrollViewer)
            .Y;
        return elementTop >= 0
            && elementTop < ContentScrollViewer.ScrollableHeight
            && elementBottom >= 0
            && elementBottom < ContentScrollViewer.ScrollableHeight;
    }
    ...
    private void ScrollToOffset(double offset)
    {
        ...
        Dispatcher.CurrentDispatcher.BeginInvoke(
            ...
            new Action(async () =>
            {
                try
                {
                    await ScrollAnimation.AnimateVertical(
                        ContentScrollViewer,
                        offset,
                        scrollCts.Token
                    );
    ...
    private void SectionListBoxItem_PreviewMouseLeftButtonDown(
        object sender,
    ...
    private void WarningsListBox_MouseDoubleClick(object sender, MouseButtonEventArgs e)
    {
        if (e.ChangedButton != MouseButton.Left)
            return;
        if (
            WarningsListBox.SelectedItem is BuildWarning buildWarning
            && buildWarning.RecordKey != null
        )
