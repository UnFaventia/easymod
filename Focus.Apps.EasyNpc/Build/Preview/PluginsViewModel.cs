...
{
    public static bool IsSuspiciousMasterCategory(PluginCategory category) =>
        category switch
        {
...
public class PluginsViewModel
{
    ...
    [DependsOn(nameof(MasterPlugins))]
    public IEnumerable<PluginViewModel> SuspiciousMasters =>
        MasterPlugins.Values.Where(x => x.IsSuspiciousMaster);
    ...
    public IEnumerable<SummaryItem> SummaryItems =>
        new List<SummaryItem>
        {
            ...
            new(
                SuspiciousMasterCount > 0
                    ? SummaryItemCategory.StatusWarning
                    : SummaryItemCategory.StatusOk,
                "Suspicious masters",
                SuspiciousMasterCount
            ),
    ...
    public PluginsViewModel(
        ...
        IPluginCategorizer pluginCategorizer,
    ...
    )
    {
        ...
        Observable
            .CombineLatest(profile.Npcs.Select(x => x.DefaultOptionObservable))
        ...
        Observable
            .CombineLatest(profile.Npcs.Select(x => x.FaceOptionObservable))
            .SubscribeOn(NewThreadScheduler.Default)
            .SubscribeSafe(
                log,
                faceOptions =>
    ...
    private PluginViewModel DescribePlugin(string pluginName)
    {
        ...
        var providingComponent = modRepository
            .SearchForFiles(pluginName, false)
