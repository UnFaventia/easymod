...
public class AssetsViewModel : IDisposable
{
    ...
    private static readonly RecordType[] NonCriticalSourceRecordTypes = new[]
    {
        RecordType.HeadPart,
    };
    ...
    [DependsOn(nameof(MissingAssets))]
    public IEnumerable<SummaryItem> AlertSummaryItems =>
        new SummaryItem[]
        {
    ...
    private readonly ConcurrentDictionary<IRecordKey, IEnumerable<AssetReference>> npcAssets =
        new();
    ...
    public AssetsViewModel(
        ...
        ICompressionEstimator compressionEstimator,
        ILogger log,
        Profile profile,
            ...
            .SubscribeSafe(
                ...
                npc =>
                {
                    ...
                    npcAssets[chain.Key] = chain.Contains(npc.FaceOption.PluginName)
                        ? chain[npc.FaceOption.PluginName].Analysis.ReferencedAssets
                        : Enumerable.Empty<AssetReference>();
    ...
    private FaceGenInfo GetFaceGenInfo(INpc npc)
    {
        ...
        var candidateComponents = npc.FaceGenOverride is not null
            ...
            : modRepository
                ...
                .ToList();
        var faceGenResult = modRepository
            .SearchForFiles(candidateComponents, faceGenPath, true)
            .FirstOrDefault();
        var faceTintResult = modRepository
            .SearchForFiles(candidateComponents, faceTintPath, true)
            .FirstOrDefault();
    ...
    {
        return (ulong)
            Math.Round(asset.size * compressionEstimator.EstimateCompressionRatio(asset.path));
    }
    ...
    private long GetFileSize(ModSearchResult? result)
    {
        ...
        return !string.IsNullOrEmpty(result.ArchiveName)
            ? archiveProvider.GetArchiveFileSize(
                fs.Path.Combine(result.ModComponent.Path, result.ArchiveName),
                result.RelativePath
            )
            : fs
    ...
    {
        var (sharedUncompressedSize, sharedCompressedSize) = npcAssets
            .Values.SelectMany(refs => refs)
            .Distinct()
            ...
            .Aggregate(
                ...
                (sums, value) =>
                    (
                        sums.uncompressed + value.size,
                        sums.compressed + EstimateCompressedSize(value)
                    )
            );
        var (faceGenUncompressedSize, faceGenCompressedSize) = npcFaceGens
            .SelectMany(pair =>
                ...
                {
                    (
                        path: FileStructure.GetFaceMeshFileName(pair.Key),
                        size: pair.Value.FaceGenSizeBytes
                    ),
                    (
                        path: FileStructure.GetFaceTintFileName(pair.Key),
                        size: pair.Value.FaceTintSizeBytes
                    ),
                }
            )
            .Aggregate(
        ...
        MissingAssets = npcAssets
            ...
            .Select(g => new MissingAssetViewModel(
                g.Key,
                g.Select(key => profile.TryGetNpc(key, out var npc) ? npc : null).NotNull()
            ))
            .OrderBy(x => x.Kind)
