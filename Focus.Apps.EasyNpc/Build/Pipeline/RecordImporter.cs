...
public class RecordImporter
{
    ...
    private readonly Lazy<
        IReadOnlyList<IFormLinkGetter<IArmorAddonGetter>>
    > raceTransformationAddons;

    public RecordImporter(
        IMutableGameEnvironment<ISkyrimMod, ISkyrimModGetter> environment,
        IAppSettings appSettings,
        ISkyrimMod mergedMod,
        ILogger log
    )
    {
        ...
        var raceTransformationKeys = appSettings
            .RaceTransformationKeys.Select(key => key.ToFormKey())
            .ToList();
        raceTransformationAddons = new(
            () => GetDefaultRaceArmorAddons(raceTransformationKeys).ToList(),
            true
        );
    }

    public void AddArmorRace(
        IFormLinkGetter<IArmorGetter> armorLink,
        IFormLinkGetter<IRaceGetter> originalRace,
    ...
    )
    {
        ...
        if (
            armorKey.IsNull
            || armorKey.ModKey != mergedMod.ModKey
            || originalRace.FormKey == actualRace.FormKey
        )
            return;
        var armor = GetIfMerged<Armor>(armorKey);
        if (armor is null)
            return;
        var affectedAddons = armor
            .Armature.Select(x => GetIfMerged<ArmorAddon>(x.FormKey))
            .NotNull()
            .Where(x =>
                x.Race.FormKey == originalRace.FormKey
                || x.AdditionalRaces.Contains(originalRace.FormKey)
            );
        foreach (var addon in affectedAddons)
            if (
                addon.Race.FormKey != actualRace.FormKey
                && !addon.AdditionalRaces.Contains(actualRace.FormKey)
            )
    ...
    public void AddHeadPartRace(
        IFormLinkGetter<IHeadPartGetter> headPartLink,
    ...
    public FormKey? Import<T, TGetter>(
        IFormLinkGetter<TGetter> link,
        Func<ISkyrimMod, IGroup<T>> groupSelector
    )
    ...
    public T? TryResolve<T>(FormKey key)
    ...
    private T? GetIfMerged<T>(FormKey key)
        ...
        where TGetter : class, ISkyrimMajorRecordGetter
    {
        ...
        {
            log.Debug(
                "Record {FormKey} is already provided by the merged plugin's masters.",
                link.FormKey
            );
            return link.FormKey;
        }
    ...
    {
        log.Debug(
            "Processing art object {FormKey} '{EditorId}' for cloning",
            art.FormKey,
            art.EditorID
        );
        ...
        log.Information(
            "Finished processing cloned art object {FormKey} '{EditorId}'",
            art.EditorID,
            art.FormKey
        );
    }
    ...
    private void ImportHeadPartDependencies(HeadPart headPart)
    {
        ...
        headPart.ValidRaces.SetTo(
            Import(headPart.ValidRaces, mergedMod.FormLists, ImportEmptyFormList)
        );
    ...
    {
        log.Debug(
            "Processing armor addon {FormKey} '{EditorId}' for cloning",
            addon.FormKey,
            addon.EditorID
        );
        ...
        var racesToKeep = addon
            .AdditionalRaces.Prepend(addon.Race)
            .NotNull()
            .Select(x => x.FormKey)
        ...
        addon.ArtObject.SetTo(
            Import(addon.ArtObject, mergedMod.ArtObjects, ImportArtObjectDependencies)
        );
        ...
        {
            var maleSkinTexture =
                Import(addon.SkinTexture?.Male, mergedMod.TextureSets) ?? FormKey.Null;
            var femaleSkinTexture =
                Import(addon.SkinTexture?.Female, mergedMod.TextureSets) ?? FormKey.Null;
        ...
        {
            var maleSwapList =
                Import(addon.TextureSwapList?.Male, mergedMod.FormLists, ImportTextureSetList)
                ?? FormKey.Null;
            var femaleSwapList =
                Import(addon.TextureSwapList?.Female, mergedMod.FormLists, ImportTextureSetList)
                ?? FormKey.Null;
        ...
        log.Information(
            "Finished processing cloned armor addon {FormKey} '{EditorId}'",
            addon.EditorID,
            addon.FormKey
        );
    }
    ...
    {
        log.Debug(
            "Processing armor {FormKey} '{EditorId}' for cloning",
            armor.FormKey,
            armor.EditorID
        );
        ...
        armor.TemplateArmor.SetTo(
            Import(armor.TemplateArmor, mergedMod.Armors, ImportWornArmorDependencies)
        );
        ...
        log.Information(
            "Finished processing cloned armor {FormKey} '{EditorId}'",
            armor.EditorID,
            armor.FormKey
        );
    }
        ...
        where TGetter : class, ISkyrimMajorRecordGetter
    {
        ...
        return link.ResolveAllContexts<ISkyrimMod, ISkyrimModGetter, T, TGetter>(
                environment.LinkCache
            )
    ...
    private void ReplaceList<T, TGetter>(
        ExtendedList<IFormLinkGetter<TGetter>>? list,
        IGroup<T> group,
        Action<T>? setup = null
    )
        where T : SkyrimMajorRecord, TGetter
        where TGetter : class, ISkyrimMajorRecordGetter
    {
        if (list is null)
            return;
        var mergedItems = list.Select(x => Import(x, group, setup)).NotNull().ToList();
        list.Clear();
