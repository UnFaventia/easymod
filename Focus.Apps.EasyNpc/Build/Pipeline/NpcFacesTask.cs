...
public class NpcFacesTask : BuildTask<NpcFacesTask.Result>
{
    ...
    public delegate NpcFacesTask Factory(
    ...
    protected override Task<Result> Run(BuildSettings settings)
    {
        return Task.Run(() =>
        {
            ...
            {
                log.Debug(
                    "Applying visual attributes for {npcLabel}",
                ...
                if (!settings.Profile.TryResolveTemplate(unresolvedModel, out var model))
                {
                    log.Warning(
                        ...
                        unresolvedModel.DefaultOption.Analysis.TemplateInfo?.Key,
                        unresolvedModel.DescriptiveLabel
                    );
                    continue;
                }
                ...
                log.Debug(
                    "Importing shallow overrides from {pluginName}",
                    model.FaceOption.PluginName
                );
                // "Deep copy" doesn't copy dependencies, so we only do this for non-referential attributes.
                record.DeepCopyIn(
                    faceNpcRecord,
                    new Npc.TranslationMask(defaultOn: false)
                    {
                        FaceMorph = true,
                        FaceParts = true,
                ...
                if (
                    faceNpcRecord.Configuration.Flags.HasFlag(
                ...
                record.HairColor.SetTo(
                    patch.Importer.Import(faceNpcRecord.HairColor, x => x.Colors)
                );
                log.Debug("Importing face texture from {pluginName}", model.FaceOption.PluginName);
                record.HeadTexture.SetTo(
                    patch.Importer.Import(faceNpcRecord.HeadTexture, x => x.TextureSets)
                );
                ...
                record.WornArmor.SetTo(
                    patch.Importer.Import(skinKey.ToLinkGetter<IArmorGetter>(), x => x.Armors)
                );
