...
public class ArchiveCreationTask : BuildTask<ArchiveCreationTask.Result>
{
    ...
    public delegate ArchiveCreationTask Factory(
        PatchSaveTask.Result patch,
        FaceGenCopyTask.Result faceGens,
        DewiggifyFaceGensTask.Result faceGenDewiggify,
        SharedResourceCopyTask.Result headParts,
    ...
    public ArchiveCreationTask(
        ...
        IDummyPluginBuilder dummyPluginBuilder,
        PatchSaveTask.Result patch,
        FaceGenCopyTask.Result faceGens,
        DewiggifyFaceGensTask.Result faceGenDewiggify,
        SharedResourceCopyTask.Result headParts,
    ...
    protected override async Task<Result> Run(BuildSettings settings)
    {
        ...
        var meshProgressSize =
            ...
            * (
                ...
                + headParts.MeshPaths.Count
                + headParts.MorphPaths.Count
            );
        var textureProgressSize =
        ...
        var meshesTask = Task.Run(
            () =>
                BuildFilteredArchive(
        ...
        var archiveFileNames = meshesTask
            .Result.ArchiveResults.Concat(texturesTask.Result.ArchiveResults)
            .Select(x => x.FileName);
        foreach (var archiveFileName in archiveFileNames)
        {
            ...
            if (archiveBaseName != baseName && archiveBaseName != $"{baseName} - Textures")
                dummyPluginBuilder.CreateDummyPlugin(
                    fs.Path.ChangeExtension(archiveFileName, ".esp")
                );
        }
    ...
    )
    {
        ...
        return new ArchiveBuilder(ArchiveType.SSE)
            .AddDirectory(
                fs.Path.Combine(outputDirectory, settings.RelativePath),
            ...
            .OnPacked(entry =>
                NextItemSync(
                    ...
                    FileStructure.IsFaceGen(entry.PathInArchive)
                        ? settings.FaceGenProgressWeight
                        : settings.DefaultProgressWeight
                )
            )
