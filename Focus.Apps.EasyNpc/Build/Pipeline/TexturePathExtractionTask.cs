...
public class TexturePathExtractionTask : BuildTask<TexturePathExtractionTask.Result>
{
    ...
    private static readonly Regex TexturePathExpression =
        new(
            @"[\w\s\p{P}]+\.dds",
    ...
    protected override async Task<Result> Run(BuildSettings settings)
    {
        ...
        var pathsFromTextureSets = patch
            ...
            .ThrottledSelect(
                async path =>
                {
                    ...
                    var extractionTask = Task.Run(
                        () => GetReferencedTexturePaths(absolutePath, fileCts.Token)
                    );
                            ...
                            .Catch(
                                (TimeoutException ex) =>
                                {
                                    log.Error(
                                        ...
                                        path,
                                        settings.TextureExtractionTimeoutSec
                                    );
    ...
    )
    {
        ...
        var fileText = await fs
            .File.ReadAllTextAsync(nifFileName, cancellationToken)
            .ConfigureAwait(false);
