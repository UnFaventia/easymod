...
public class DewiggifyRecordsTask : BuildTask<DewiggifyRecordsTask.Result>
{
    ...
    public delegate DewiggifyRecordsTask Factory(
        PatchInitializationTask.Result patch,
        NpcDefaultsTask.Result defaults,
        NpcFacesTask.Result faces
    );
    ...
    public DewiggifyRecordsTask(
        IReadOnlyGameEnvironment<ISkyrimModGetter> env,
        IWigResolver wigResolver,
        ILogger log,
        PatchInitializationTask.Result patch,
        NpcDefaultsTask.Result defaults,
        NpcFacesTask.Result faces
    ...
    protected override Task<Result> Run(BuildSettings settings)
    {
        ...
        return Task.Run(() =>
        {
            var wigs = defaults.Npcs.Select(x => x.Model.FaceOption.Analysis.WigInfo).NotNull();
            var wigMatches = wigResolver
                .ResolveAll(wigs)
                .Where(x => x.HairKeys.Count > 0)
                .ToDictionary(x => x.WigKey, x => x.HairKeys);
            var npcsWithMatchedWigs = defaults
                .Npcs.Where(x =>
            ...
            foreach (var (model, record) in npcsWithMatchedWigs)
            {
                ...
                var oldHairParts = record
                    .HeadParts.Select(x => patch.Importer.TryResolve<IHeadPartGetter>(x.FormKey))
                    .NotNull()
                ...
                var mergedHair = patch.Importer.Import(
                    hairKey.ToFormKey().ToLink<IHeadPartGetter>(),
                    x => x.HeadParts
                );
                if (mergedHair.HasValue)
                    record.HeadParts.Add(mergedHair.Value);
                wigConversions.Add(
                    new NpcWigConversion
                    {
                        Key = new RecordKey(model),
                        HairColor = GetHairColor(record),
                        AddedHeadParts = mergedHair.HasValue
                            ? DescribeHeadParts(mergedHair.Value, patch.Importer)
                                .ToList()
                                .AsReadOnly()
    ...
    private IEnumerable<HeadPartInfo> DescribeHeadParts(FormKey formKey, RecordImporter importer)
    {
        ...
        return headPart
            .ExtraParts.SelectMany(x => DescribeHeadParts(x.FormKey, importer))
            .Prepend(
                new HeadPartInfo
                {
    ...
    private Color? GetHairColor(INpcGetter npc)
    {
        if (npc.HairColor.IsNull)
            return null;
        var colorRecord = npc.HairColor.TryResolve(
            env.LinkCache,
            npc,
            log,
            "This NPC will have the wrong hair color."
        );
