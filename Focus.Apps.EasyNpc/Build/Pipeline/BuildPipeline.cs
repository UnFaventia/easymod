        ...
        where T : Delegate
    {
        ...
        if (invokeMethod is null)
            throw new ArgumentException(
                $"Delegate {typeof(T).FullName} does not have an Invoke method.",
                nameof(T)
            );
        var resultType = GetResultType(invokeMethod);
        if (resultType is null)
            throw new ArgumentException(
                $"Delegate {typeof(T).FullName} does not return a type of {nameof(IBuildTask<T>)}.",
                nameof(T)
            );
...
class BuildInstance<TSettings, TResult> : IBuildProgress<TResult>
{
    ...
    public BuildInstance(
        ...
        ILogger log,
        IEnumerable<TaskRegistration> registrations,
        TSettings settings
    )
    {
        this.container = container;
    ...
    private void Continue()
    {
        ...
        try
        {
            ...
            foreach (var reg in remainingRegistrations)
            {
                ...
                if (args.All(x => x is not null))
                {
                    ...
                    task.ItemName.TakeUntil(endedSubject)
                        .Subscribe(itemName =>
                            log.Debug("{taskName} -> {itemName}", task.Name, itemName)
                        );
            ...
            {
                var remainingRegistrationTypeNames = remainingRegistrations.Select(x =>
                    x.Factory.GetType().FullName
                );
                throw new BuildException(
    ...
    private void Fail(IBuildTask task, Exception? ex)
    {
        ...
        outcomeSource.TrySetException(
            new BuildException(
                $"Task '{task.Name}' ({task.GetType().FullName}) failed to complete",
                ex
            )
        );
        Cleanup();
    ...
    private Task StartAndWatch(IBuildTask buildTask)
    {
        ...
        var runtimeTask = Task.Run(async () =>
        {
            ...
            {
                log.Warning(
                    "Task '{taskName}' started, but didn't return a valid Task instance",
                    buildTask.Name
                );
        ...
        runtimeTask.ContinueWith(
            t =>
            {
                log.Information(
                    "Task '{taskName}' ended with status {taskStatus}",
                    buildTask.Name,
                    t.Status
                );
                ...
                switch (t.Status)
                {
                    ...
                    default:
                        throw new BuildException(
                            $"Unexpected task status on continuation: {t.Status}"
                        );
                }
            },
...
class BuildPipeline<TSettings, TResult> : IBuildPipeline<TSettings, TResult>
{
    ...
    public IBuildProgress<TResult> Start(TSettings settings)
    {
        ...
        var build = new BuildInstance<TSettings, TResult>(
            container,
            contextLog,
            registrations,
            settings
