...
public class FaceGenCopyTask : BuildTask<FaceGenCopyTask.Result>
{
    public class Result
    {
        ...
        public Result(
            IReadOnlyCollection<string> meshPaths,
            IReadOnlyCollection<string> tintPaths,
        ...
        return Task.Run(() =>
        {
            ...
            var found = settings
                ...
                .Where(x =>
                    ...
                    || (
                        !x.FaceOption.IsBaseGame
                        && x.FaceOption.PluginName != x.DefaultOption.PluginName
                    )
                )
                .SelectMany(x =>
                    new[]
                    ...
                    }.Select(p => new
                    {
                        ...
                        Components = (
                            x.FaceGenOverride is not null
                                ? x.FaceGenOverride.Components
                                : ComponentsForPlugin(x.FaceOption.PluginName)
                        ).ToHashSet(),
                    })
                )
            ...
            Parallel.ForEach(
                ...
                x =>
                {
                    ...
                    if (!string.IsNullOrEmpty(x.ArchiveName))
                    {
                        ...
                        wasCopied = archiveProvider.CopyToFile(
                            archivePath,
                            x.RelativePath,
                            outputPath
                        );
                    ...
                    if (wasCopied)
                    {
                        if (
                            fs
                                .Path.GetExtension(x.RelativePath)
                                .Equals(".nif", StringComparison.OrdinalIgnoreCase)
                        )
                            lock (meshPaths)
