...
{
    private static readonly Dictionary<string, VanillaRace> termToRace =
    ...
    private static readonly Dictionary<string, TargetSex> termToSex =
        new(StringComparer.OrdinalIgnoreCase)
        {
            { "f", TargetSex.Female },
    ...
    private static readonly Regex raceRegex =
        new(
            @$"[\W_]({string.Join("|", termToRace.Keys)})[\W_]",
            RegexOptions.Compiled | RegexOptions.IgnoreCase
        );
    private static readonly Regex sexRegex =
        new(
            @$"[\W_]({string.Join("|", termToSex.Keys)})[\W_]",
            RegexOptions.Compiled | RegexOptions.IgnoreCase
        );

    private ILookup<string, RecordAnalysisChain<HeadPartAnalysis>> hairs = Enumerable
        .Empty<RecordAnalysisChain<HeadPartAnalysis>>()
        .ToLookup(x => "");
    ...
    public void Receive(LoadOrderAnalysis analysis)
    {
        ...
        hairs = headPartChains
            .Where(x =>
                x.Master.IsMainPart
                && x.Master.PartType == HeadPartType.Hair
    ...
    public IEnumerable<WigMatch> ResolveAll(IEnumerable<NpcWigInfo> wigs)
    {
        ...
        foreach (var wig in wigs)
        {
            ...
            var targetRace = raceMatch.Success
                ? termToRace[raceMatch.Groups[0].Value]
                : VanillaRace.Nord;
            ...
            var targetSex = sexMatch.Success
                ? termToSex[sexMatch.Groups[0].Value]
                : TargetSex.Unknown;
    ...
    private static bool MatchesRace(
        RecordAnalysisChain<HeadPartAnalysis> hairChain,
        VanillaRace race
    )
    {
