...
public class PostBuildReportGenerator
{
    ...
    public PostBuildReportGenerator(
        ...
        IModSettings modSettings,
        IGameSetup setup,
        Lazy<IGameSettings> gameSettings,
        IFileSystem fs,
        IArchiveProvider archiveProvider,
        Lazy<ILoadOrderAnalyzer> loadOrderAnalyzer,
        Lazy<IFaceGenEditor> faceGenEditor
    )
    {
        ...
        vanillaComponent = new ModComponentInfo(
            ModLocatorKey.Empty,
            "Vanilla",
            "Vanilla",
            setup.DataDirectory
    ...
    public async Task<PostBuildReport> CreateReport()
    {
        ...
        var modRepositoryConfigTask = Task.Run(
            () => modRepository.Configure(new(modSettings.RootDirectory))
        );
        var archiveIndexTask = Task.Run(() =>
        {
            var archivePaths = gameSettings.Value.ArchiveOrder.Select(f =>
                fs.Path.Combine(gameSettings.Value.DataDirectory, f)
            );
        ...
        var analysisTask = Task.Run(
            () =>
                loadOrderAnalyzer.Value.Analyze(
                    setup.AvailablePlugins.Select(x => x.FileName),
                    setup.LoadOrderGraph,
                    true
                )
        );
        ...
        headParts = analysis
            .ExtractChains<HeadPartAnalysis>(RecordType.HeadPart)
        ...
        var mergeComponents = modRepository
            .SearchForFiles(mainPluginName, false)
        ...
        {
            MainPluginMissingMasters = setup
                .LoadOrderGraph.GetMissingMasters(mainPluginName)
                .ToList()
                .AsReadOnly(),
    ...
    {
        var regex = new Regex(
            $@"^{fs.Path.GetFileNameWithoutExtension(mainPluginName)}( - Textures)?\d*$",
            RegexOptions.Compiled
        );
        var archiveFilePaths = new Dictionary<string, string>(
            StringComparer.CurrentCultureIgnoreCase
        );
        var pluginFilePaths = new Dictionary<string, string>(
            StringComparer.CurrentCultureIgnoreCase
        );
        foreach (var fileName in fs.Directory.EnumerateFiles(setup.DataDirectory))
        {
            ...
            if (archiveProvider.IsArchiveFile(fileName))
                ...
                fs.Path.GetExtension(fileName).Equals(".esp", StringComparison.OrdinalIgnoreCase)
            )
    ...
    private async Task<NpcConsistencyInfo> CheckNpc(
        RecordAnalysisChain<NpcAnalysis> chain,
        string mainPluginName,
        ModComponentInfo? mergeComponent
    )
    {
        ...
        {
            var mergedFaceGens = modRepository
                .SearchForFiles(new[] { mergeComponent }, faceGenPath, true)
                .ToList();
            ...
            faceGenLoosePath = mergedFaceGens
                ...
                .FirstOrDefault();
            var mergedFaceTints = modRepository
                .SearchForFiles(new[] { mergeComponent }, faceTintPath, true)
                .ToList();
        ...
        return new NpcConsistencyInfo
        {
            ...
            HasConsistentFaceTint =
                ...
                string.IsNullOrEmpty(faceGenArchivePath)
                || string.IsNullOrEmpty(faceGenLoosePath)
                || faceTintSource?.ModComponent == faceGenSource?.ModComponent,
            HasConsistentHeadParts = await HasConsistentHeadParts(
                winner,
                pluginSource,
                faceGenSource
            ),
    ...
    private AssetSource? FindAssetSource(string assetPath, bool checkArchives)
    {
        ...
        if (!checkArchives)
            return null;
        var containingArchiveName = archiveIndex
            .FindInBuckets(assetPath)
        ...
        if (!string.IsNullOrEmpty(containingArchiveName))
        {
            ...
            var archiveSource = archiveSources.GetOrAdd(
                ...
                _ =>
                    FindAssetSource(
                        containingArchiveName,
                        fs.Path.Combine(settings.DataDirectory, containingArchiveName)
                    )
            );
    ...
    private PluginState GetPluginState(string fileNameOrPath)
    {
        ...
        return setup.LoadOrderGraph.CanLoad(fileName)
            ? PluginState.Enabled
            : PluginState.Unloadable;
    }

    private async Task<bool> HasConsistentHeadParts(
        NpcAnalysis npc,
        AssetSource? pluginSource,
        AssetSource? faceGenSource
    )
    {
        ...
        if (faceGenSource.ModComponent == pluginSource.ModComponent)
            ...
            return true;
        var recordHeadParts = npc
            .MainHeadParts.Select(x => headParts.GetOrDefault(x))
            .NotNull()
            .ToList();
        var extraPartKeys = recordHeadParts.SelectMany(x => x.ExtraPartKeys);
        while (extraPartKeys.Any())
        {
            var extraParts = extraPartKeys
                .Select(x => headParts.GetOrDefault(x))
                .NotNull()
                .ToList();
    ...
    private bool IsMainArchive(string archivePath, string mainBaseName)
    {
        ...
        return fileName.Equals(mainBaseName, StringComparison.CurrentCultureIgnoreCase)
            || fileName.Equals(
                $"{mainBaseName} - Textures",
