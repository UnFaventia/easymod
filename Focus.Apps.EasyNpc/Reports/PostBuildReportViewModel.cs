...
public class PostBuildReportViewModel
{
    ...
    public bool HasAllDummyPluginsEnabled =>
        Report.Archives.All(x =>
            !x.RequiresDummyPlugin || x.DummyPluginState == PluginState.Enabled
        );
    ...
    public PostBuildReportViewModel(
        PostBuildReportGenerator reportGenerator,
        ArchiveExtractorViewModel.Factory archiveExtractorFactory
    )
    {
    ...
    public async Task UpdateReport()
    {
        ...
        if (HasSingleMergeComponent)
        {
            ...
            var faceGenFiles = InconsistentHeadPartNpcs
                ...
                .Select(x => new ArchiveFile(
                    x.FaceGenArchivePath!,
            ...
            var faceGenTintFiles = InconsistentHeadPartNpcs
                ...
                .Select(x => new ArchiveFile(
                    x.FaceTintArchivePath!,
                    FileStructure.GetFaceTintFileName(x)
                ));
            if (faceGenFiles.Any())
                FaceGenArchiveExtractor = archiveExtractorFactory(
                    faceGenFiles.Concat(faceGenTintFiles),
                    mergeComponent
                );
