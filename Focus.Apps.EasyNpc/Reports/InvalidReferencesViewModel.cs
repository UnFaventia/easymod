...
public class InvalidReferenceViewModel
{
    ...
    public InvalidReferenceViewModel(INpc npc, NpcOption option)
    {
        ...
        Paths = option
            .Analysis.InvalidPaths.Select(p => new ReferencePathViewModel(p))
            .ToList()
            .AsReadOnly();
    }
}
...
{
    public IEnumerable<ReferenceInfoViewModel> References =>
        path.References.Select(
            (x, i) => new ReferenceInfoViewModel(x, i < (path.References.Count - 1))
        );
