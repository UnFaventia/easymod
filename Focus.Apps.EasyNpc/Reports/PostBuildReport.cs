...
public class PostBuildReport
{
    ...
    public IReadOnlyList<MergeArchiveInfo> Archives { get; init; } =
        new List<MergeArchiveInfo>().AsReadOnly();
    public IReadOnlyList<string> MainPluginMissingMasters { get; init; } =
        new List<string>().AsReadOnly();
    public string MainPluginName { get; init; } = FileStructure.MergeFileName;
    public PluginState MainPluginState { get; init; }
    public IReadOnlyList<NpcConsistencyInfo> Npcs { get; init; } =
        new List<NpcConsistencyInfo>().AsReadOnly();
}
