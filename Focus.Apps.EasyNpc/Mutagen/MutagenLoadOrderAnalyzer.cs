...
public class MutagenLoadOrderAnalyzer : LoadOrderAnalyzer
{
    ...
    {
        var referenceChecker = new ReferenceChecker<INpcGetter>(groupCache).Configure(
            ConfigureNpcReferences
        );
        var assetPathConfig = AssetPathConfiguration
            .Builder()
            .For<IArmorAddonGetter>(armorAddon =>
                armorAddon
                    .Add(AssetKind.Mesh, x => x.FirstPersonModel, x => x.File)
                    .Add(AssetKind.Mesh, x => x.WorldModel, x => x.File)
            )
            .For<IArmorGetter>(armor =>
                armor
            ...
            .Build();
        var assetPathExtractor = new AssetPathExtractor<INpcGetter>(
            groupCache,
            assetPathConfig
        ...
        runner
            .Configure(
                RecordType.Npc,
                new NpcAnalyzer(groupCache, referenceChecker, assetPathExtractor)
            )
                ...
                addon =>
                    addon
                        ...
                        .Follow(
                            ...
                            artObject =>
                                artObject.Follow(x =>
                                    x.Model?.AlternateTextures?.Select(t => t.NewTexture)
                                )
                        )
                        .Follow(
                            x => x.FirstPersonModel,
                            g => g.AlternateTextures?.Select(x => x.NewTexture)
                        )
                        .Follow(x => x.Race)
                        ...
                        .Follow(
                            ...
                            swapList =>
                                swapList.Follow(x =>
                                    x.Items.Where(x => x.Type == typeof(ITextureSetGetter))
        ...
        follower
            ...
            .Follow(
                ...
                race =>
                    race.Follow(
                            x => x.HeadData,
                            g => g.HeadParts.Select(x => x.Head),
                            ConfigureHeadPartReferences
                        )
