...
class Program
{
    ...
    private static void Run(Options options)
    {
        var loggingLevelSwitch = new LoggingLevelSwitch(
            options.Verbose ? LogEventLevel.Debug : LogEventLevel.Information
        );
        ...
        var baseDirectory = !string.IsNullOrEmpty(options.DirectoryPath)
            ...
            : Path.Combine(env.DataFolderPath, "data");
        var faceGenDirectory = Path.Combine(
            baseDirectory,
            "meshes",
            "actors",
            "character",
        ...
        var faceGenProcessor = new FaceGenProcessor(
            env,
            fs,
            faceGenDirectory,
            tempFileCache,
...
class FaceGenProcessor
{
    ...
    public FaceGenProcessor(
        ...
        IFileSystem fs,
        string faceGenDirectory,
        TempFileCache tempFileCache,
        FaceGenProcessingOptions options,
        ILogger log
    )
    {
        this.env = env;
        ...
        var gameSettings = GameSettings.From(
            GameEnvironmentWrapper.Wrap(env),
            new(GameRelease.SkyrimSE)
        );
    ...
    {
        return Directory
            .GetDirectories(faceGenDirectory)
            .SelectMany(dir =>
                Directory
                    .GetFiles(dir, "*.nif")
    ...
    private void FixFaceTints(NifFile file, NiNode faceGenNode, string modName, string formId)
    {
        ...
        {
            var defaultTintPath =
                $@"textures\actors\character\facegendata\facetint\{modName}\{formId}.dds";
            ReplaceTexturePath(
                file,
                faceShape,
                TextureSlot.InnerLayerDiffuse,
                defaultTintPath,
                IsNullOrMissing
            );
        }
    ...
    private static bool ReplaceTexturePath(
        NifFile file,
        BSDynamicTriShape shape,
        TextureSlot slot,
        string newPath,
        Predicate<string> oldPathCondition
    )
    {
        var shader = shape.HasShaderProperty()
            ? file.GetHeader().GetBlockById(shape.ShaderPropertyRef().index)
                as BSLightingShaderProperty
            : null;
        if (shader == null)
            return false;
        var textureSet = shader.HasTextureSet()
            ? file.GetHeader().GetBlockById(shader.TextureSetRef().index) as BSShaderTextureSet
            : null;
        if (textureSet == null)
            return false;
        ...
        if (
            !string.IsNullOrEmpty(oldTexture)
    ...
    {
        return env
            .LoadOrder.PriorityOrder.HeadPart()
            .WinningOverrides()
            .Where(x => x.Type == type)
            .Select(x => x.EditorID)
            .NotNullOrEmpty()
    ...
    {
        var path = tempFileCache.GetTempPath(
            fileProvider,
            Path.Combine("meshes", skeletonFileName)
        );
        using var file = new NifFile();
        file.Load(path);
