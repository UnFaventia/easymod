...
public class VortexModRepository
    : ComponentPerDirectoryModRepository<ComponentPerDirectoryConfiguration>
{
    ...
    {
        return filePriorities.TryGetValue(relativePath, out var priority)
            ? Prioritize(results, priority)
            : results;
    }
    ...
    private static bool MatchesFilePriority(ModSearchResult result, FilePriority priority)
    {
        return string.IsNullOrEmpty(result.ArchiveName)
            && result.ModComponent.Name.Equals(
                priority.WinningFileId,
