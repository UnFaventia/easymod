...
public class VortexComponentResolver : IComponentResolver
{
    ...
    public Task<ModComponentInfo> ResolveComponentInfo(string componentName)
    {
        ...
        var mod =
            file is not null && !string.IsNullOrEmpty(file.ModId)
                ? manifest.Mods.GetOrDefault(file.ModId)
                : null;
        ...
        return Task.FromResult(
            new ModComponentInfo(key, componentId, componentName, componentPath, isEnabled)
        );
    }
}
