...
public class AnalysisRunnerTests
{
    ...
    {
        logger = new LoggerConfiguration().MinimumLevel.Debug().WriteTo.Debug().CreateLogger();
        ...
        runner = new AnalysisRunner(
            recordScannerMock.Object,
            availablePlugins,
            loadOrderGraphMock.Object,
            logger
        );
    }
    ...
    public void WhenAnalyzerTypeDifferentFromRecordType_Configure_Throws()
    {
        ...
        Assert.Throws<ArgumentException>(
            () => runner.Configure(RecordType.Armor, weaponAnalyzer.Object)
        );
    }
    ...
    public void ForAllPlugins_ReportsMasters()
    {
        ...
        loadOrderGraphMock
            .Setup(x => x.GetAllMasters("plugin3", true))
            .Returns(new[] { "plugin1", "implicit1" });
    ...
    public void ForAllRecords_RunsConfiguredAnalyzers()
    {
        ...
        SetupLoadOrderGraph(
            new() { { "plugin1", Enumerable.Empty<string>() }, { "plugin2", new[] { "plugin1" } } }
        );
        SetupRecordScanner(
            "plugin1",
            new()
    ...
    public void WhenPluginAnalysisFails_ProvidesEmptyAnalysisWithException()
    {
        ...
        SetupRecordScanner(
            "plugin1",
            new() { { RecordType.Keyword, new[] { "keyword1:plugin1" } } }
        );
        SetupRecordScanner(
            "plugin2",
            new() { { RecordType.Keyword, new[] { "keyword2:plugin2" } } }
        );
        SetupRecordScanner(
            "plugin3",
            new() { { RecordType.Keyword, new[] { "keyword3:plugin3" } } }
        );
        SetupRecordScanner(
            "plugin4",
            new() { { RecordType.Keyword, new[] { "keyword4:plugin4" } } }
        );
        var analyzerMock = new Mock<IRecordAnalyzer<RecordAnalysis>>();
        analyzerMock.SetupGet(x => x.RecordType).Returns(RecordType.Keyword);
        analyzerMock
            .Setup(x => x.Analyze(It.IsAny<string>(), It.IsAny<IRecordKey>()))
            .Returns(
                (string pluginName, IRecordKey key) =>
        ...
        var loadOrderAnalysis = runner.Configure(RecordType.Keyword, analyzerMock.Object).Run();
    ...
    public void WhenRecordTypesIgnored_ExcludesFromAnalysis()
    {
        ...
        SetupRecordScanner(
            ...
            new()
            {
                {
                    RecordType.Armor,
                    new[] { "armor1:plugin1", "armor2:plugin2", "armor3:plugin3" }
                },
        ...
        var includedRecordTypes = loadOrderAnalysis
            .Plugins.SelectMany(x => x.Groups)
            .Where(x => x.Value.Records.Count > 0)
            .Select(x => x.Key)
            .Distinct();
        var includedEditorIds = loadOrderAnalysis
            .Plugins.SelectMany(x => x.Groups)
    ...
    private Mock<IRecordAnalyzer<RecordAnalysis>> DefaultAnalyzerMock(
        RecordType recordType,
        string editorIdPrefix
    )
    {
    ...
    private void SetupLoadOrderGraph(Dictionary<string, IEnumerable<string>> plugins)
    {
        ...
        loadOrderGraphMock
            .Setup(x => x.GetMissingMasters(It.IsAny<string>()))
            .Returns(Enumerable.Empty<string>());
        foreach (var plugin in plugins)
        {
            loadOrderGraphMock
                .Setup(x => x.GetAllMasters(plugin.Key, It.IsAny<bool>()))
                .Returns(plugin.Value);
            loadOrderGraphMock.Setup(x => x.IsEnabled(It.IsAny<string>())).Returns(true);
        }
    }

    private void SetupRecordScanner(
        string pluginName,
        Dictionary<RecordType, IEnumerable<string>> recordKeys
    )
    {
        foreach (var item in recordKeys)
        {
            recordScannerMock
                .Setup(x => x.GetKeys(pluginName, item.Key))
