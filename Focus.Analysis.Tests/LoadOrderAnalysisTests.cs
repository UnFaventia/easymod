...
public class LoadOrderAnalysisTests
{
    ...
    private PluginAnalysis DummyPluginAnalysis(
        string fileName,
        params RecordAnalysisGroup[] recordGroups
    )
    {
        return new PluginAnalysis(fileName) { Groups = recordGroups.ToDictionary(x => x.Type) };
    }

    private RecordAnalysisGroup DummyRecordAnalysisGroup(
        RecordType type,
        int count,
...
static class ChainAssertions
{
    public static void ChainHas(
        this RecordAnalysisChain<FakeRecordAnalysis> chain,
        string editorId,
    ...
    )
    {
        ...
        Assert.Collection(
            chain,
            pluginNames
                .Select<string, Action<Sourced<FakeRecordAnalysis>>>(p =>
                    x =>
                    {
                        Assert.Equal(p, x.PluginName);
                    }
                )
                .ToArray()
        );
